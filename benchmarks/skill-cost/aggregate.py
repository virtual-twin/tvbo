...
def main() -> None:
    ...
    ap.add_argument("--only-success", action="store_true",
                    help="Aggregate only rows where success=True.")
    ap.add_argument("--out-prefix", default=None,
                    help="Output path prefix (default: alongside --results).")
    args = ap.parse_args()
    ...
    agg = grp.agg(
        **{col: (col, "median") for col, _ in METRICS},
        ...
        n=("success", "size"),
    ).reset_index().sort_values(["harness", "model", "condition"])

    # ---- Markdown table (mirrors the reference layout) -------------------
    headers = ["Harness", "Model", "Condition"] + [label for _, label in METRICS] \
        + ["Success", "N"]
    lines = ["| " + " | ".join(headers) + " |",
             "|" + "|".join(["---"] * len(headers)) + "|"]
    for _, r in agg.iterrows():
        ...
        cells = [
            str(r["harness"]), str(r["model"]), str(r["condition"]),
            f"{r['processed_tokens']:.0f}", f"{r['output_tokens']:.0f}",
            f"{r['tool_calls']:.0f}", f"{r['wall_seconds']:.1f}", cost_s,
            f"{r['success_rate'] * 100:.0f}%", f"{int(r['n'])}",
        ]
    ...
    prefix = Path(args.out_prefix) if args.out_prefix else \
        args.results.with_name("skill_cost")
...
def _plot(agg, out_path: Path) -> None:
    ...
    conditions = [c for c in CONDITION_ORDER
                  if c in set(agg["condition"].astype(str))]
    ...
    for i, (harness, model) in enumerate(combos):
        ...
        proc = [sub.loc[c, "processed_tokens"] if c in sub.index else np.nan
                for c in conditions]
