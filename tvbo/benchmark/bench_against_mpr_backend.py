#
# Module: bench_against_mpr_backend.py
#
# Author: Leon Martin
...
class Bench(object):
    ...
    COLW = [
        len(col) - 2 for col in LINE.split("+")[1:-1]
    ]  # the widths of columns based on the first header line
    _FS = " | ".join(
        "%" + str(cw) + "s" for cw in COLW
    )  # builds a format string like  "| %6s | %6s | %6s ... "
    FS = "| " + _FS + " |"
                    ...
                    for length in self.sim_lengths:
                        for dt in self.int_dts:
                            for conduction in self.conductions:
                                for integrator in self.integrations:
                                    try:
                                        sim = simulator.Simulator()
                                        sim.network = conn
                                        sim.model = model
                                        sim.integration = integrator
                                        sim.integration.dt = dt
                            ...
                            for conduction in self.conductions:
                                for integrator in self.integrations:
                                    timestr = time.strftime(
                                        "%H:%M:%S", time.gmtime(self.running_times[i])
                                    )
                                    print(
                                        self.FS
                                        % (
...
def read_connectivity(file_path):
    ...
    conn = connectivity.Connectivity(
        ...
        region_labels=numpy.array(
            numpy.zeros(numpy.shape(weights_orig)[0]), dtype="<U128"
        ),
        ...
        centres=numpy.zeros(numpy.shape(weights_orig)[0]),
    )  # default 76 regions
    conn.configure()
...
def main():
    ...
    )  # read_connectivity("SC_Schaefer7NW100p_nolog10.txt")
    connectivities = [conn]
