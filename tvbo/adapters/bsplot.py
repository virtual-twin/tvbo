...
def _resolve_layer(layer, panel_kind, base_dir):
    ...
    # value, so a layer-wide colour or label would collide with the per-entry ones. Every mark
    # with its own branch ABOVE the colour fan-out (heatmap/scatter/bar/area/band/rule) draws a
...
def _used_source(used) -> tuple:
    ...
    Split rather than pre-joined so a panel drawing several outputs of ONE analysis (a
    density with its mean and a reference line on it) names that analysis once. An output
...
def _panel_descriptor(panel) -> str:
    ...
    Clauses are deduplicated. A grid draws one binding per CELL — the same frames laterally and
    then medially, one analysis per row — so listing them undeduplicated repeated a single source
