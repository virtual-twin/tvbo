...
def _figure_block(workflow, overrides, engine: str = "snakemake"):
    ...
    Reuses the ``_workflow`` merge machinery so the semantics match the experiment emitter exactly: name-keyed engine slots (env/options) merge by name, and the engine block inherits the engine-agnostic resource keys (and, for Snakemake, the
...
def _rule_resources(block: dict) -> dict:
    ...
    Returns ``{key: python-literal-string}`` (already repr'd so the template emits ``key=<literal>`` verbatim). ``cpus_per_task``/``mem_mb``/``runtime`` map across engines; the SLURM scheduler identity (partition/account/gres) is surfaced as the executor's ``slurm_partition``/``slurm_account``/``slurm_extra`` resources so a per-figure override lands on the rule itself. ``options`` pass through verbatim.
    """
    r: dict = {}
