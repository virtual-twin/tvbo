...
UNITS = {
    ...
    "kohm": ("resistance", 3),
    "Mohm": ("resistance", 6),
    "ohm_cm": ("resistivity", -2),
    "kohm_cm": ("resistivity", 1),
    "m": ("length", 0),
    "cm": ("length", -2),
    "um": ("length", -6),
    "mol_per_m3": ("concentration", 0),
    "mol_per_cm3": ("concentration", 6),
    "M": ("concentration", 3),
    "mM": ("concentration", 0),
    "per_s": ("per_time", 0),
    "per_ms": ("per_time", 3),
...
def _dynamics_has_time_units(params, svs, dvs):
    ...
    return (
        any(unit_has_time_dimension(getattr(params[k], "unit", None)) for k in td_params if k in params)
        or any(unit_has_time_dimension(getattr(sv, "unit", None)) for sv in svs.values())
...
_UNIT_TO_DIMENSION = {
    ...
    "uS": "conductance",
    "pS": "conductance",
    "S_per_cm2": "conductance_density",
    "mS_per_cm2": "conductance_density",
    # temperature
    "degC": "temperature",
    # current
    "A": "current",
...
def _render_custom_component_type(dynamics, role_slot=None):
    ...
    if "TIME_SCALE" not in params:
        lines.append('        <Constant name="TIME_SCALE" dimension="time" value="1 ms"/>')
    ...
    for p_name, p_val in params.items():
        ...
        if not nml_val:
            continue
        lines.append(f'        <Constant name="{p_name}" dimension="{dim}" value="{nml_val}"/>')

    # Requirements
    for req_name, req_dim in reqs.items():
        lines.append(f'        <Requirement name="{req_name}" dimension="{req_dim}"/>')
    ...
    for dv_name, dv in dvs.items():
        ...
        if cases is not None:
            ...
            if is_exposure:
                ...
                lines.append(f'            <DerivedVariable name="{dv_name}" dimension="none" value="{rhs}"/>')
    lines.append("        </Dynamics>")
...
def _render_cell_xml(dyn, dyn_id=None, custom_types=None):
    ...
    if not input_xmls:
        ...
        for suffix in ("", "_2", "_3", "_4"):
            ...
            if params.get(d_key) and params.get(dur_key) and params.get(amp_key):
                ...
                input_refs.append(f'        <explicitInput target="pop[0]" input="{pid}" destination="synapses"/>')
    ...
    if cell_type == "pointCellCondBased":
        ...
        cell_lines.append(f'    <pointCellCondBased id="{dyn_id}" C="{C_attr}" v0="{v0_attr}" thresh="{thresh_attr}">')
        ...
        init_v = _nml_attr(params.get("initMembPotential") or params.get("v0"), "-65.0 mV")
        ...
        if segments:
            ...
            seg_list = sorted(segments.items(), key=lambda x: int(getattr((x[1].parameters or {}).get("id"), "value", 0)))
            for seg_name, seg in seg_list:
                ...
                parent_id = int(getattr(parent_param, "value", -1)) if parent_param else -1
                cell_lines.append(f'            <segment id ="{seg_id}" name="{seg_name}">')
                if parent_id >= 0:
                    cell_lines.append(f'                <parent segment="{parent_id}"/>')
            ...
            for grp_name, grp in segment_groups.items():
                ...
                nlid = getattr(nlid_param, "label", None) if nlid_param else None
                nlid_attr = f' neuroLexId="{nlid}"' if nlid else ""
                cell_lines.append(f'            <segmentGroup id="{grp_name}"{nlid_attr}>')
                for pn, pv in gp.items():
                    if pn in ("neuroLexId", "members", "includes"):
                        continue
                    val = getattr(pv, "value", pv)
                    if val is not None:
                        cell_lines.append(f'                <property tag="{pn}" value="{int(val)}"/>')
                ...
                if members_param:
                    ...
                    for mid in str(members_str).split(","):
                        mid = mid.strip()
                        if mid:
                            cell_lines.append(f'                <member segment="{mid}"/>')
                includes_param = gp.get("includes")
                if includes_param:
                    includes_str = getattr(includes_param, "shape", "") or ""
                    for inc in str(includes_str).split(","):
                        inc = inc.strip()
                        if inc:
                            cell_lines.append(f'                <include segmentGroup="{inc}"/>')
...
def _build_hier_custom_context(experiment):
    ...
    for ch_key, ch_dyn in (dyn.modes or {}).items():
        ...
        ch_inst_params = {k: v for k, v in ch_params.items() if k not in ("number", "erev")}
        ...
        for g_key, g_dyn in (ch_dyn.modes or {}).items():
            ...
            for r_key, r_dyn in (g_dyn.modes or {}).items():
                ...
                if r_type_name not in type_defs:
                    r_ct_params = [
                        {"name": k, "dimension": _param_dimension(k, v)} for k, v in r_params.items() if k not in r_inherited
                    ]
            ...
            if g_type_name not in type_defs:
                g_ct_params = [
                    {"name": k, "dimension": _param_dimension(k, v)} for k, v in g_params.items() if k not in g_inherited
                ]
    ...
    cell_params = {k: v for k, v in (dyn.parameters or {}).items() if k not in ("pulse_delay", "pulse_duration", "I_amp")}
    cell_inherited = root_meta.get("inherited_params", set())
    cell_ct_params = [
        {"name": k, "dimension": _param_dimension(k, v)} for k, v in cell_params.items() if k not in cell_inherited
    ]
...
def build_std_lems_context(experiment):
    ...
    if network:
        ...
        if nodes and edges and dynamics_lib:
            has_nml_cells = any(_uses_neuroml_types(d) for d in dynamics_lib.values() if hasattr(d, "iri"))
            if has_nml_cells:
...
def _build_std_network_context(experiment):
    ...
    for dyn_name, group_nodes in groups.items():
        ...
        if has_positions:
            for node in group_nodes:
                ...
                if pos:
                    node_positions.append((pos.x or 0, pos.y or 0, pos.z or 0))
                else:
                    node_positions.append((0, 0, 0))

        seg_ids = cell_result.get("segment_ids", []) if cell_result else []
    ...
    for edge_idx, edge in enumerate(edges):
        ...
        if syn_key not in synapse_set:
            ...
            elif any(s_id == safe_id(syn_type) for s_id in synapse_set.values()):
            ...
            if resolved_syn_dyn:
                ...
                for pk, pv_str in syn_params.items():
                    attr_parts.append(f'{pk}="{pv_str}"')
                synapse_xmls.append(f"    <{syn_type} {' '.join(attr_parts)}/>")
    ...
    needs_inputs_include = bool(input_nodes) or any(p.get("is_input") for p in populations)
    ...
    has_segment_targeting = any(inp.get("segmentId") is not None for inp in explicit_inputs)
    needs_input_list = (
        has_weighted
        or has_segment_targeting
        or any(pop_map.get(inp["target_pop"], {}).get("is_populationlist") for inp in explicit_inputs)
    )
...
def build_lems_context(experiment):
    ...
    _bodies = function_bodies(dyn.functions, parameters=all_names)

    def lems_expr(e):
        """Parse (if needed), inline model functions, then print as LEMS.

        LEMS has no user-defined function mechanism, so every call must be expanded to its
    ...
    if net_ctx:
        ...
        for ct_name, ct_dyn in net_ctx["cell_types"].items():
            ...
            def _make_ct_lems_expr(ct_dyn, ct_all_names, ct_fn_names):
                ct_bodies = function_bodies(ct_dyn.functions, parameters=ct_all_names)
    ...
    * ``export(dir)``         → write file(s) to disk, optionally validate
    ...
    def export(self, dir, format="lems", split=False, validate=True, **kwargs) -> dict:
        ...
        if format.lower() in ("neuroml", "nml"):
            ...
            lems_xml = self.render_lems_wrapper(neuroml_file=nml_name, **kwargs)
    ...
    def run(self, backend="jneuroml", **kwargs) -> "ExperimentResult":
        ...
        **kwargs
        ...
        if backend not in self._BACKENDS:
            raise ValueError(f"Unknown NeuroML backend {backend!r}. Supported: {', '.join(sorted(self._BACKENDS))}")

        uses_std = self.experiment and self.experiment.dynamics and _uses_neuroml_types(self.experiment.dynamics)
        ...
        if self.experiment:
            ...
            if network:
                ...
                if _net_nodes and _net_edges and _net_dyn_lib:
                    if any(_uses_neuroml_types(d) for d in _net_dyn_lib.values() if hasattr(d, "iri")):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if not dat_files:
                raise RuntimeError(f"Backend {backend!r} produced no .dat output files in {tmpdir}")
            ...
            if len(dat_files) == 1:
                ...
                if _is_std_network and _std_net_output_pops:
                    ...
                    for pop_id, pop_size, sv_var in _std_net_output_pops:
                        ...
                        cell_files = sorted(
                            [s for s in dat_by_stem if re.match(rf"^{re.escape(comp)}_\d+$", s)],
                        ...
                        if cell_files:
                            ...
                            for stem, arr in dat_by_stem.items():
                                if stem not in matched_stems and (stem.startswith(comp) or comp.startswith(stem)):
                                    value_parts.append(arr[:, 1:])
                                    matched_stems.add(stem)
                                    break
                    if not value_parts:
                        value_parts = [arr[:, 1:] for arr in dat_by_stem.values()]
                else:
                    value_parts = [arr[:, 1:] for arr in dat_by_stem.values()]
            ...
            try:
                ...
                for _of in _root.iter():
                    if _of.tag.endswith("OutputFile") or _of.tag == "OutputFile":
                        for _oc in _of:
                            if _oc.tag.endswith("OutputColumn") or _oc.tag == "OutputColumn":
                                q = _oc.get("quantity", "")
                                if q:
                                    col_names.append(q)
            except Exception:
    ...
    def _invoke_runner(backend: str, lems_file, tmpdir):
        ...
        if not success:
            raise RuntimeError(f"pyNeuroML runner {runner_name}() failed for {lems_file.name} (backend={backend!r})")
...
def _resolve_nml2_root() -> _Path:
    ...
    if env_dir:
        ...
        if (p / "LEMSexamples").is_dir():
            _nml2_root_cache = p
            return p
        raise FileNotFoundError(f"NEUROML2_DIR={env_dir} does not contain LEMSexamples/")
...
def run_lems_example(lems_file: str, cwd: str | _Path | None = None) -> dict[str, _np.ndarray]:
    ...
    def _run_neuron_backend(lems_name: str) -> _subprocess.CompletedProcess:
        ...
        jnml = _shutil.which("jnml") or _shutil.which("jnml", path=str(_Path(_sys.prefix) / "bin"))
        if not jnml:
    ...
    if result.returncode != 0:
        ...
        needs_neuron = "MULTICOMPARTMENTAL_CELL_MODEL" in text or "requires Neuron" in text or "Ex25" in lems_file
        ...
        if result.returncode != 0:
            merged = "\n".join([result.stdout or "", result.stderr or ""])
            raise RuntimeError(f"jNeuroML failed (rc={result.returncode}):\n{merged[-4000:]}")
    ...
    if not outputs:
        ...
        try:
            if _inject_probe_output(src, probe_path):
                ...
                if result.returncode != 0:
                    raise RuntimeError(f"jNeuroML probe run failed (rc={result.returncode}):\n{result.stderr[-2000:]}")
                outputs = _collect_outputs()
        finally:
...
def compare_traces(
    ...
    for col_name in ref_cols:
        ...
        f_tvbo = interp1d(tvbo_time, tvbo_trace_raw, kind="linear", fill_value="extrapolate")
        ...
        print(f"  {col_name}: RMSE={rmse:.6f}  max_err={max_err:.6f}  corr={corr:.6f}  {status}")
...
def plot_comparison(
    ...
    for i, name in enumerate(sv_names):
        ...
        ax.plot(ref_time, ref_data[:, ref_idx], label="NeuroML (ref)", alpha=0.8)
...
def _match_quantity_to_col(quantity: str, tvbo_cols: list[str]) -> str | None:
    ...
    if idx_match:
        ...
        candidates = [c for c in tvbo_cols if idx in c and c.endswith("/" + var_suffix)]
        if len(candidates) == 1:
            return candidates[0]
...
def _find_ref_column(quantity: str, ref_outputs: dict, output_columns: dict | None = None) -> tuple[str, int] | None:
...
def parse_lems_output_columns(lems_file: str) -> dict[str, list[str]]:
    ...
    for of in sim.findall("OutputFile"):
        ...
        quantities = [oc.attrib.get("quantity", "") for oc in of.findall("OutputColumn")]
    ...
    for display in displays:
        ...
        for line in display.lines:
            ...
            if ref_info:
                ...
                ax.plot(t_ref, y_ref, color=color, alpha=0.9, linewidth=1.5, label=f"{label} (ref)")
                has_ref = True
            ...
            if tvbo_col and tvbo_time is not None:
                ...
                ax.plot(t_tvbo, y_tvbo, color=color, alpha=0.5, linewidth=1.5, linestyle="--", label=f"{label} (TVBO)")
                has_tvbo = True
        ...
        legend_handles = []
        if has_ref:
            legend_handles.append(Line2D([0], [0], color="gray", linewidth=1.5, label="NeuroML ref (solid)"))
        if has_tvbo:
            legend_handles.append(
                Line2D([0], [0], color="gray", linewidth=1.5, linestyle="--", alpha=0.6, label="TVBO (dashed)")
            )
        if legend_handles:
