...
A curated ``GraphGenerator`` under ``tvbo/database/graph_generators/`` is defined by its ``procedure:`` block: an ordered DAG of typed steps whose options are schema
fields. :mod:`tvbo.graph_generators.procedural` resolves that DAG to SymPy and renders it through the printer tables in ``tvbo/codegen/code.py``, so eager construction at
``Network`` load time and emitted backend source are the same expressions rendered twice. There are no per-generator Python materialisers (see
``dev/GenericProcedureEngine.md``).

Two kinds of generator sit outside the DAG, both through the standard ``bindings`` slot: library wrappers (Graphs.jl / NetworkX families) and the documented Callable
exception below, for a construction the backend-independent primitive set genuinely cannot express.
...
def weight_shuffle(source: str, preserve: str = "binary_mask", seed: Optional[int] = None) -> dict:
    ...
    ``dev/GenericProcedureEngine.md`` §5): a masked extract, a permutation and a scatter are not expressible in the backend-independent primitive set. Boolean-mask extraction
    in particular cannot survive expression parsing at all — ``M[M != 0]`` evaluates its comparison to a plain Python ``True`` before an expression tree is ever built. So the
    ...
    ``preserve='binary_mask'`` keeps the ``{0, nonzero}`` pattern and permutes the weight values among their existing positions, so density and topology are held fixed while
