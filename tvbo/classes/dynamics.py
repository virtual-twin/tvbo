...
The public import location for :class:`Dynamics`, alongside the helpers that read a model
out of the ontology.

There is no wrapper class: a model's own methods live in
[`tvbo.behaviour.dynamics`](../behaviour/dynamics.qmd) and are attached to the generated
class itself, so a `Dynamics` carries them however it was built — loaded through LinkML,
validated through Pydantic, or resolved onto an edge. The equations live on a
[`SymbolicSystem`](../parse/system.qmd), one per model.
...
See the [writing-models](../../../skills/writing-models/SKILL.md) skill for the YAML form
and equation conventions.
"""
