...
# Only a fully-qualified `network.*` value is a reference; anything else is a literal (a bare `weight` is a state variable, not the connectome) — the same rule the pipeline argument path uses, so a producer and a pipeline step read arguments identically.
_REF_PREFIX = "network."
...
def _resolve_ref(ref: str, context: Any, where: str) -> Any:
    ...
    # The context is the owning experiment, or a network directly (handy for tests and for a caller that already has one). Discriminate on the slot, not on whether nodes happen to be populated — an empty network is still a network.
...
def _hashable(value: Any) -> Any:
    ...
    if isinstance(value, np.ndarray):
        # dtype and shape too: `tobytes()` alone collides (2,3) with (3,2), and float32 with the int32 of the same bytes, onto one key and one artifact path.
...
def _module_source_digest(module: str) -> str:
    ...
    try:
        ...
        # Uncached, so a transient read failure degrades this one lookup rather than pinning the digest-free key for the rest of the process.
        return ""
...
def _call_producer(producer: Any, param_name: str, context: Any) -> Any:
    ...
    if output:
        # A callable returning several named arrays (e.g. a bundle of mesh operators) is selected by `output`, so one precompute serves many parameters and runs once.
        try:
        ...
        # Hand back a shallow copy of the bundle: its arrays are read-only, but the dict itself is the cache entry, and rebinding a key in it would poison every other parameter reading this producer.
...
def _write_bundle(path: Path, produced: Any) -> None:
    ...
    # A temp path unique to this process: several cluster shards materialise the same artifact concurrently, and a shared temp would let one truncate another's file mid-write and then both rename, publishing a corrupt artifact that every later shard reads as a valid cache hit. The rename itself is atomic, so readers only ever see a complete file.
