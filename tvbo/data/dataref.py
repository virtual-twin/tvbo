...
A ``DataRef`` (``schema/common.yaml``) points at one labelled array in another
experiment's result, a dataset, or a curated entity: WHERE (``experiment`` id or
``iri``), WHICH (``output``), SLICE (``sel``), and an optional node ``reconcile``.
It is the single primitive behind four authoring surfaces — a figure ``Layer.used``,
a sourced ``Argument.used``, a sourced ``Parameter.used``, and (via the same
container + label semantics) the ``initial_state.from_experiment`` seed. This module
is their shared resolver, so "find the source container, take the array, slice it,
...
``xarray.DataArray`` and takes its network context (label alias map + model node
order) by injection, so both the run-time experiment resolvers and the figure codegen
...
def experiment_id(iri) -> Optional[str]:
    ...
    ``tvbo:exp/<study>/exp-32`` / ``exp-32`` / ``exp32`` / a bare ``32`` -> ``"32"``. A
    curated / dataset iri whose last segment merely *contains* digits
    (``rec-avgMatrix_atlas-HCPMMP1``) returns ``None`` — so it is not misread as an
    experiment (which would silently bind to ``exp-1``); ``locate_container`` then treats
...
def locate_exp_container(results_root, source_id) -> Path:
    ...
    Globs by the ``exp-<id>_`` file stem, skipping the ``*network*`` sidecar, so the
    output-directory layout (``results/2``, ``output/nc/exp2``, flat BIDS files, …)
...
def is_local_ref(ref) -> bool:
    ...
    Such a reference names one of *this* experiment's own outputs (the local end of
    the reference spectrum, subsuming an ``Argument.value: "observations.x"``); it is
...
def select_labeled(da, sel: Optional[Mapping[str, object]]):
    ...
    Each entry selects along a dimension by coordinate label (``method="nearest"`` for
    a numeric selection, e.g. the sampled K on a continuous sweep; exact for a label).
    A selection key that is a *non-dimension* coordinate defined along a single
    dimension is honoured too (the operating-point-out-of-a-branch case: the container
    is dimmed by ``branch_point`` with ``K`` a coordinate along it) — the nearest
    index along that coordinate is taken. Returns ``da`` unchanged for an empty sel.
    """
    if not sel:
        return da
    import numpy as np
    ...
    for dim, value in sel.items():
        if dim in da.dims:
            ...
            if _is_numeric(value):
                # Nearest match along the coordinate — for a scalar (drops the axis) AND
                # for a list (keeps the axis, one nearest index per target), consistent
                # with the dimension path's method="nearest". Exact np.isin would silently
                # miss on a continuous sweep whose sampled values never match exactly.
...
def reconcile_by_label(da, alias_map: Mapping[str, str], model_labels: Sequence[str],
    ...
    source -> canonical through ``alias_map`` (alias-aware, so a divergent nomenclature
    ...
    ``node_dims`` to restrict reconciliation to a known set of node axes (so a
    labelled *non-node* dimension is not mistaken for one); the default reconciles
    every string-coordinate axis. This is the ``reconcile: by_label`` path; ``none``
    skips it.
    """
    import numpy as np
    ...
    for d in list(da.dims):
        ...
        # Without an explicit node_dims restriction, only reconcile an axis that is
        # actually a node axis — its labels overlap the model's node labels. A labelled
        # NON-node axis (e.g. a 'population'/'variable' coord) is left untouched rather
        # than forced through `.sel(model_labels)` (which would raise).
...
def resolve_dataref(
    ...
    *,
    ...
    Runs the four steps in order — WHERE (:func:`locate_container`), WHICH
    (:func:`match_output`), SLICE (:func:`select_labeled`), RECONCILE
    (:func:`reconcile_by_label`, only when the reference asks for ``by_label`` and a
    network context is supplied) — and returns the array detached from the source
    ...
    For a *local* reference (no WHERE) raises via :func:`locate_container`; callers
    test :func:`is_local_ref` first and route those to the in-run resolver.
    """
    import xarray as xr
...
def apply_transform(da, name: Optional[str]):
    ...
    Resolves the name against the shared ``bsplot`` transform registry — the same one a
    figure ``Layer.transform`` and a study's ``code_modules`` use — so a sourced array and
    a figure layer name the same transforms. Imported lazily to keep this module free of an
    adapter dependency; a miss raises the registry's actionable "not registered" error.
    """
    if not name:
        return da
