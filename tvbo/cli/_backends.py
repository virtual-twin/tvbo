...
BACKENDS: dict[str, BackendSpec] = {
    "jax": BackendSpec(
        ...
        tasks=frozenset({"ODEIntegration", "SDEIntegration",
                         "ParameterExploration", "GradientBasedOptimization"}),
        capabilities=frozenset({"Autodiff", "JITCompilation", "GPUSupport",
                                "VectorizedRNG", "CodeGeneration"}),
        vectorize_axes=frozenset({"parameters", "initial_conditions", "noise_seed"}),
    ),
    "tvboptim": BackendSpec(
        name="tvboptim", label="tvboptim",
    ...
    "tvb": BackendSpec(
        name="tvb", label="TVB",
        tasks=frozenset({"ODEIntegration", "DDEIntegration",
                         "SDEIntegration", "SDDEIntegration", "ParameterExploration"}),
        capabilities=frozenset({"NumPyExecution", "BuiltinModelLibrary",
                                "DelayHistoryBuffer", "StochasticSolver"}),
        vectorize_axes=frozenset(),  # everything fans out
    ),
    "networkdynamics": BackendSpec(
        name="networkdynamics", label="NetworkDynamics.jl",
        tasks=frozenset({"ODEIntegration", "DDEIntegration",
                         "SDEIntegration", "SDDEIntegration", "ParameterExploration"}),
        capabilities=frozenset({"JuliaJIT", "DiffEqIntegrators",
                                "DelayHistoryBuffer", "StochasticSolver", "StiffSolver"}),
        vectorize_axes=frozenset({"parameters"}),
        aliases=("nd",),
    ),
    "bifurcationkit": BackendSpec(
        name="bifurcationkit", label="BifurcationKit.jl",
