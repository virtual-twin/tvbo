...
def build(
    tractogram: Path = typer.Argument(
        ...,
        help="Streamline tractogram MRtrix can read (e.g. .tck). Must be in the "
             "same space as the parcellation.",
    ),
    parcellation: Path = typer.Argument(
        ...,
        help="Integer-labelled parcellation image (e.g. dseg.nii.gz), in the same "
             "space as the tractogram.",
    ),
    ...
    atlas: Optional[str] = typer.Option(
        None, "--atlas", help="Parcellation/atlas name (metadata + filename entity)."),
    ...
    cohort: Optional[str] = typer.Option(
        None, "--cohort", help="Cohort/dataset entity (e.g. HCPYA)."),
    reconstruction: Optional[str] = typer.Option(
        None, "--reconstruction", "--rec",
        help="Tractography pipeline name (e.g. dTOR); recorded as the tractogram and "
             "reconstruction entity."),
    segmentation: Optional[str] = typer.Option(
        None, "--seg", help="Segmentation entity (e.g. 17Networks)."),
    scale: Optional[str] = typer.Option(
        None, "--scale", help="Scale entity (e.g. 1000)."),
    labels: Optional[Path] = typer.Option(
        None, "--labels",
        help="Optional node labels: a text file with one label per line, ordered by "
             "parcellation label (1..N)."),
    symmetric: bool = typer.Option(
        True, "--symmetric/--no-symmetric", help="Pass -symmetric to tck2connectome."),
    zero_diagonal: bool = typer.Option(
        True, "--zero-diagonal/--no-zero-diagonal",
        help="Pass -zero_diagonal to tck2connectome."),
    keep_assignments: Optional[Path] = typer.Option(
        None, "--keep-assignments",
        help="Also save tck2connectome's -out_assignments to this path."),
    mrtrix_arg: Optional[List[str]] = typer.Option(
        None, "--mrtrix-arg",
        help="Extra raw argument forwarded to both tck2connectome calls (repeatable)."),
    overwrite: bool = typer.Option(False, "--overwrite", help="Overwrite existing output."),
    dry_run: bool = typer.Option(
        False, "--dry-run",
        help="Print the tck2connectome commands and exit without running them."),
) -> None:
    ...
    if dry_run:
        ...
        for cmd in tck2connectome_commands(
            tractogram, parcellation,
            tmp / "weights.csv", tmp / "lengths.csv",
            (tmp / "assignments.csv") if keep_assignments else None,
            symmetric=symmetric, zero_diagonal=zero_diagonal, extra_args=extra,
        ):
            typer.echo(" ".join(cmd))
        return
