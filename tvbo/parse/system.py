...
class SymbolicSystem:
    ...
    def form(self, notation: str = "symbol", evaluate: bool = True):
        ...
        The cache is discarded whole whenever [`_inputs`](#tvbo.parse.system.SymbolicSystem) changes, which is what makes it safe on a mutable model. Rendering is a query — `render_code` does not run `update_metadata` — so no consumer can invalidate it mid-use.
