...
A *streaming reducer* replaces the ``O(window * N^2)``/step rolled-buffer recompute of a sliding-window source observation (e.g. ``compute_fc`` recomputed
over the whole BOLD window every online-tuning iteration) with an incremental
``O(N^2)``/step reducer over the ``(add, evict, emit, resync)`` protocol. The reducer is authored declaratively (:class:`StreamingReducerSpec`) and lowered to
backend source by :mod:`tvbo.codegen.reducers`, so tvbo emits the realization into the generated experiment — no backend ships one.

The registry is **per backend**: a backend that has a registered streaming form for a given pipeline reducer uses the streaming path in generated code; a backend
that registers nothing (``tvb`` / ``pyrates`` / ``julia``) keeps the recompute path byte-for-byte unchanged. The whole mechanism is transparent to the recipe /
...
Registering a new streaming reducer (e.g. for dFC / metastability) is a new
``database/reducers/*.yaml`` recipe — state + add/evict/resync/emit assignment strings — loaded at import; no code change.
"""
...
class StreamingReducerSpec:
    ...
    The reducer's math is authored as symbolic **assignment recurrences** (strings over the state vars, the arriving/leaving sample ``v``, and the window ``x``); the
    general resolver in :mod:`tvbo.codegen.reducers` parses them and the sympy printers lower them per backend, so tvbo emits the reducer into the generated experiment and
    no backend ships a reducer realization (works on any backend/tvboptim version — nothing external to resolve). **Sequential reassignment encodes the data flow** — a
    later RHS sees the value assigned to a state var above it — so no ``new_*`` temporaries are needed. The recipe is data here; the template that emits it is
    fully general.
...
def lookup_streaming_reducer(backend: str, reducer_module: str | None, reducer_name: str):
    ...
    Returns ``None`` (-> recompute fallback) when the backend registers nothing for this reducer, or when streaming is disabled via the
    ``TVBO_STREAMING_REDUCERS=0`` environment switch (used to codegen the recompute reference for validation and as an escape hatch). The reducer
    realization is tvbo-emitted, so there is no external factory to resolve.
    """
...
def streaming_capable(backend: str, reducer_module: str | None, reducer_name: str) -> bool:
    ...
    The algorithm template gates its streaming branch on this; ``"stride"`` (dFC / FCD) specs return ``False`` here until the template's stride branch
...
def is_windowed_reducer(reducer_module: str | None, reducer_name: str, backend: str = "tvboptim") -> bool:
    ...
    The registered reducer (``compute_fc`` for ``tvboptim``) is a windowed correlation reduction — undefined over a ``< 2``-sample window, where
    ``jnp.corrcoef`` collapses to a scalar — so codegen routes it through a degenerate-window guard. Unlike :func:`lookup_streaming_reducer`, this is a pure
    registry-membership test with no factory-availability or
    ``TVBO_STREAMING_REDUCERS`` gating: the guard applies whether or not the streaming *path* is active. It shares the one registered reducer set so the
