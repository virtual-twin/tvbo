...
``O(N^2)``/step reducer over the ``(add, evict, emit, resync)`` protocol. The
reducer is authored declaratively (:class:`StreamingReducerSpec`) and lowered to
backend source by :mod:`tvbo.codegen.reducers`, so tvbo emits the realization
into the generated experiment — no backend ships one.
...
class StreamingReducerSpec:
    ...
    The reducer's math is authored as symbolic **assignment recurrences** (strings
    over the state vars, the arriving/leaving sample ``v``, and the window ``x``); the
    general resolver in :mod:`tvbo.codegen.reducers` parses them and the sympy printers
    lower them per backend, so tvbo emits the reducer into the generated experiment and
    no backend ships a reducer realization (works on any backend/tvboptim version —
    nothing external to resolve). **Sequential reassignment encodes the data flow** — a
    later RHS sees the value assigned to a state var above it — so no ``new_*``
    temporaries are needed. The recipe is data here; the template that emits it is
    fully general.
...
def lookup_streaming_reducer(backend: str, reducer_module: str | None, reducer_name: str):
    ...
    recompute reference for validation and as an escape hatch). The reducer
...
def is_windowed_reducer(reducer_module: str | None, reducer_name: str, backend: str = "tvboptim") -> bool:
    ...
    correlation reduction — undefined over a ``< 2``-sample window, where
    ``jnp.corrcoef`` collapses to a scalar — so codegen routes it through a
    ...
    streaming *path* is active. It shares the one registered reducer set so the
...
# + add/evict/resync assignment strings + emit); this loads and registers each for the
# backends and pipeline reducers it declares. No reducer realisation is hardcoded here —
# adding a reducer is a YAML file, not code. Backends that register nothing (tvb / pyrates
