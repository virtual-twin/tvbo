...
"""Resolution helpers shared by the tvboptim Mako templates.

Everything a template needs resolved — parameter layouts, recorded variable names, coupling specs, streaming reducers — is worked out here in Python, so the templates stay emission-only.
...
def get_param_info(parameters: dict) -> Tuple[List[str], Dict[str, float], Dict[str, str]]:
    ...
    A parameter with no literal `value` is *absent* from the defaults rather than given one: it may be free, or sourced or produced, and resolved from storage rather than inlined. Emission sites supply their own fallback with `.get(name, 1.0)`. Inventing one here would erase the difference between "undeclared" and "1.0", and let a resolved operator emit silently as a scalar.
    ...
    # Consistent, deterministic parameter ordering (by name). Every emission site binds params by name (DEFAULT_PARAMS Bunch, params.<name>, override dicts) so order is purely cosmetic — sorting makes it stable across models and codegen paths, independent of how the source parameter collection was built.
    ...
    for p in params:
        if isinstance(p.value, (list, tuple)):
            # Array-valued constant (e.g. a mode-coupling matrix): keep the nested list; the codegen wraps it as a jnp.array, not a scalar default.
...
def get_mode_layout(model: Any) -> Tuple[int, List[str], Dict[str, List[int]]]:
    ...
    tvboptim's solver carries a 2-D state ``(n_states, n_nodes)`` and its coupling contracts the node axis with a plain matmul, so it has no place for a third per-node mode axis. A model with ``number_of_modes > 1`` (the Stefanescu-Jirsa
    ReducedSet models) folds that mode axis into the state axis: each state variable ``v`` occupies ``n_modes`` contiguous scalar slots ``v__mode0 .. v__mode{M-1}``. The dfun reconstructs the ``(n_nodes, n_modes)`` mode-vector for each variable from its slots, evaluates the mode-aware equations (``mode_dot``/``mode_sum``), and scatters the per-mode derivatives back into those slots; per-mode coupling falls out of the existing 2-D matmul because each ``(var, mode)`` slot couples to the same slot across nodes.
...
def get_recorded_variable_names(model: Any, experiment: Any = None) -> Tuple[List[str], List[str], List[str]]:
    ...
    Returns ``(state_names, recorded_aux, all_var_names)`` where ``all_var_names`` is the runtime ordering on axis 1 of ``solution.ys`` / ``result.data`` and matches ``solution.variable_names`` produced by tvboptim >= 0.2.7. The state channels follow the solver's own, possibly mode-folded, layout: with `number_of_modes > 1` each variable contributes `n_modes` scalar slots.
...
def get_output_channels(model: Any, experiment: Any = None) -> Tuple[List[int], List[str], bool]:
    ...
    # Non-state channels (auxiliaries) are always kept — they are recorded only when explicitly requested, so record_flag.get(nm, True) leaves them in.
...
def _shape_ndim(shape_str) -> int:
    ...
    Counts the comma-separated dimensions inside the parentheses so a graph-shaped (2-D, per-edge) parameter can be told apart from a per-node (1-D) or scalar one.
...
def resolve_coupling_spec(coupling, coupling_key, model, coupling_inputs_info, func_to_ci, n_modes=1) -> Dict[str, Any]:
    ...
    Vectorised or per-edge is decided here. An identity-sentinel `pre()` — no pre-expression at all, or the bare keyword `local_states` or `incoming_states`, both meaning "sources unchanged" — is a pure linear incoming sum, `gx = W @ source`, and takes the vectorised incoming-identity path however its states are declared: local-only as in `FastLinearCoupling`, incoming-only, or both on the same cvar. A source-only phase coupling with a real pre-expression stays per-edge, which is exact for any connectome, unless the coupling opts in with `vectorized: true`.
    """
    ...
    # Infer incoming_states from the pre-expression when not explicit: match model state-variable names in the RHS, else fall back to the coupling_variable states.
    ...
    # Mode fold (number_of_modes>1): a coupling reading a multi-mode cvar emits one coupled output per mode; resolve the source cvar to its n_modes per-node slots.
    ...
    # Graph-shaped (n_nodes, n_nodes) params are per-edge; tvboptim requires them declared in EDGE_PARAMS. Read from the declared shape so it holds for optimised weights.
...
def resolve_coupling_input_map(model, all_couplings, coupling_inputs_dict):
    ...
    TVB's surface/local coupling, zero for the region-based simulations tvboptim supports, so it must not be wired to the long-range connectome (the dfun binds it to 0 via its fallback).
    ...
    # Drop LOCAL coupling terms from the network wiring (they stay in coupling_inputs_dict so the dfun still binds the symbol to its 0.0 fallback).
...
def _resolve_bold_stream(obs: Any, experiment: Any = None) -> Dict[str, Any]:
    ...
    Both strides are derived from the grid, not read off the pipeline. The HRF kernel is sampled on the model's ``downsample_period`` grid, so on an integration grid with a different ``dt`` the neural signal has to be decimated onto that same grid before convolving, and the TR stride then counts downsampled samples. A pipeline's hardcoded subsample ``step`` or strided ``stride`` assumes one ``dt`` and is wrong on any other, so they are computed from ``downsample_period``, the observation's ``period`` and ``dt`` where those are available, and fall back to the declared strides only where they are not.

    The Volterra scaling ``k_1 * V_0 * (x - 1)`` takes its constants from a ``volterra_transform`` step — either a referenced Function's equation parameters, or an inline ``equation`` on the step itself, which is what the fused strided pipeline uses.
    """
    ...
    # HRF kernel: the kernel-generator step (its Function carries a `time_range`). Reuse the emitted kernel function so the streaming kernel is byte-identical to the pipeline's; forward the step's literal arguments, else the function's own defaults.
    kernel_step = next(
    ...
    # The decimation MUST be a pure subsample stride; temporal_average is an averaging window (even period_samples=1 shifts by one sample) and is not block-additive, so it cannot be streamed faithfully.
    ...
    if _period is not None and _dt:
        ...
        # Un-fused pipeline subsamples at the TR with a `subsample_bold` step (arg `s`); the fused strided pipeline folds it into the strided convolution `stride`.
        _tr = (
...
def _resolve_stat_stream(obs: Any) -> Dict[str, Any]:
    ...
    if agg == "mean":
        ...
        # ddof=0. Guard the one-pass co-moment with Abs: catastrophic f64 cancellation on a near-constant source can make it slightly negative, which two-pass jnp.var/jnp.std never return (and would NaN under the sqrt). Abs (unary → jnp.abs) is a no-op for real variance, so it stays byte-identical there while giving ~0 (not NaN) at the degenerate end.
...
def _resolve_fc_stream(obs: Any) -> Optional[Dict[str, Any]]:
    ...
    A ``pipeline: [compute_fc]`` observation — a node-node Pearson-correlation FC over a recorded source (e.g. inp_corr over the excitatory input current ``x_e_pre``) — folds into the integrator carry as a Welford co-moment accumulator: the existing ``windowed_fc`` reducer recipe's ``add`` update and zero-diagonal Pearson ``emit``,
...
# The canonical symbol an observer's expressions use for the signal it observes, bound to whatever the observation's `source` names. It is what lets an observation model be curated once and pointed at any model's state variable, the same role `x_j` plays in a curated coupling's pre_expression.
OBSERVER_INPUT = "x"
...
def resolve_reduction(obs: Any, experiment: Any = None) -> Optional[Dict[str, Any]]:
    ...
    An observation may declare a co-integrated auxiliary ``Dynamics`` (the observer) that computes it online as a time recurrence, instead of a post-scan ``pipeline``. It may instead opt a post-scan ``pipeline`` into streaming with ``reduce: streaming``, lifting it to a block reducer: a cumulative mean, standard deviation or variance over a source synthesizes a running-moment accumulator; a ``compute_fc`` windowed-correlation pipeline becomes an add-only Welford co-moment FC reducer; an HRF-Volterra BOLD pipeline is lifted by :func:`_resolve_bold_stream` and tagged ``kind: 'convolution'``. An observation that declares its own ``dynamics`` is already a reducer, and ``reduce: streaming`` on it opts that reducer into the post-tuning carry rather than selecting a pipeline-lifted one, so the observer path owns it.
    ...
    Every RHS is parsed to a **sympy** expression against the observer's symbolic vocabulary (its states, its ``parameters``, the source, and the framework scalars ``dt``/``count``; user functions become undefined ``Function``s). An observer parameter is a named constant exactly as a model Dynamics' parameters are, scalar or array-valued (a mesh operator, a template matrix), and binds by name in the rendered update. That makes the analysis symbolic, not string-based: accumulator classification is ``state_symbol in expr.free_symbols``, and an unknown symbol (a typo) is caught here rather than surfacing as a codegen error. The context carries sympy ``Expr`` objects; the partial renders them per backend via ``render_expression``, which accepts sympy directly.

    The vocabulary a RHS is parsed against is the observer's own states, its parameters, the source, and the framework scalars ``dt`` and ``count``, all as Symbols, with its user functions as undefined ``Function``s. Derived variables — the per-step intermediates — join it too, so a richer observer's DV can reference an earlier one in a detector chain such as ``pgn = pg / nrm``; a simple observer declares none and this is inert. ``derived_parameters`` are the observer's constants, functions of its parameters and ``dt`` and never of a state or the observed signal, so they bind once in the reducer's preamble instead of being recomputed per step — the same slot a model Dynamics uses for the same purpose, and what lets a readout built only from states and constants be evaluated per emitted sample rather than per step.

    A state may also declare an ``evict_equation``: the reverse recurrence that removes the sample leaving a sliding window, the inverse of ``update``, parsed against the same vocabulary with ``source`` denoting the sample being folded in or removed. Its presence is what makes the observer a *windowed* reduction — add, evict, resync — rather than a cumulative accumulator.

    The readout is the deprecated ``output`` list, or else the derived variable marked ``record: true``, which is how a Dynamics now names what it reports. Exactly one recorded variable can be the observation's value; several is ambiguous rather than a case for a default. A ``period`` on the observer makes it a *monitor*: its output is emitted every period into a time series instead of collapsing time into one value per node at the final step, which is `Observation.dynamics`'s declared contract. An ``aggregation: median`` needs explicit histogram bins spanning the reduced quantity's range — assuming a default window would clip out-of-range samples and pin the result to an edge — so the slot is required rather than guessed.

    Each constant resolves to what the template emits. A literal binds inline. A sourced or produced one is materialised at codegen time into a content-addressed artifact and emitted as a lazy ``(file, key)`` the backend reads at run time, so a large operator never enters the generated source; a relative source path is grounded against the spec dir exactly as ``bids_dir`` is. Materialisation runs the producer and writes to disk, so it happens only when an experiment is supplied — that is, at genuine emission. This function is also called bare, with no experiment, as a cheap "is this a streaming reducer?" predicate, and that path must stay side-effect-free: a lazy constant is left deferred rather than materialised to answer a boolean, and no igl precompute is triggered on the way.
    ...
    # Derived PARAMETERS are constants (see below); their names bind in every expression exactly as a parameter's does.
    dps = get_attr(dyn, "derived_parameters")
    dp_map = dict(dps.items()) if hasattr(dps, "items") else {}
    dp_names = [str(n) for n in dp_map]

    # Named constants the observer's expressions bind by name — resolved exactly as a model Dynamics' parameters are (same helper), so a scalar and an array-valued constant (e.g. a mesh operator) declare identically and need no observer-specific provenance path.
    from tvbo.data import param_io
    ...
    # Materialisation writes to disk, so it runs only at genuine emission (experiment supplied).
    ...
    # One symbol cannot denote both a per-step state and a fixed constant: the update would read whichever the renderer bound last, silently.
    ...
    # A curated observer cannot know which model state a study will point it at, so it writes its input as the canonical OBSERVER_INPUT — the device a curated Coupling already uses for its presynaptic state (`x_j`) and summed input (`gx`). It is bound here to whatever `source` names, so every rendered expression carries the real symbol and no backend template needs to know about the alias. An observer that declares its own symbol of that name means that symbol, and no alias applies.
    ...
    # The derived-variable chain, resolved as sympy exprs in declaration order (a DV may reference earlier DVs, the source, states, and parameters). A simple observer has an empty chain and reads its value straight from ``output`` below; a detector observer computes this chain per step and the renderer emits it as sequential assignments.
    derived = []
    ...
    # Permutation-significance surrogates: a derived variable may declare a `surrogate` (re-evaluate a named statistic DV under permutations of a field, report the exceedance p-value). Resolve each to the statistic EXPANDED for its permute symbol — DVs that depend on the permuted symbol are inlined so the statistic becomes a self-contained function of it, while DVs that don't (a captured observed value, e.g. U w.r.t. an in-strength permutation) stay as symbols the renderer binds in-step. General; the wave detector is one consumer.
    ...
    # Time-reduction statistic (Observation.aggregation). Default (anything but 'median') folds a running sum (the accumulator state above) and divides by count. 'median' folds a per-node HISTOGRAM of the per-step output (bins from Observation.histogram) into the carry (O(bins) memory, no trajectory) and reads the 0.5 quantile at finalize — the streaming-safe way to get the robust median instantaneous frequency Koller uses, which a running sum cannot compute.
    ...
    # Resolved against the integration grid, so it needs the experiment; the predicate call does not.
    _period = get_attr(obs, "period")
    _period = float(to_numeric(_period)) if _period is not None else None
    _dt = _integration_dt(experiment)
    period_steps = None
    ...
    # Deferrable to per-sample unless it reads the observed signal or a per-step derived variable.
    ...
    red = {
        # 'monitor' emits every period_steps into a time series; 'recurrence' folds the whole run into one value per node. Both render from the same observer context.
    ...
    if partition is not None:
        ...
        red.update(
            {
                "kind": "wave",
                # n_groups reads the produced gather table, so it needs the materialised artifact (experiment supplied). The bare predicate call (experiment=None) only asks IS this a streaming reducer, so leave it deferred there rather than force an igl precompute.
...
def streaming_post_eval_plan(experiment: Any) -> Dict[str, Any]:
    ...
    - ``deliverables``: the derived observations computable from the streamed values plus
      the static network observations ALONE — i.e. WITHOUT the raw trajectory (the FC family: ``fc`` from streamed ``bold``, then ``fc_corr``/``fc_rmse`` from ``fc`` and the empirical target). Observations that need the raw trajectory (e.g. a post-scan ``mean`` over a state variable) are intentionally absent — at fit scale they cannot be materialised anyway;
    ...
    for n, o in obs_by_name.items():
        ...
        # Fold the reducers that write into a sample-indexed carry, so no run materialises the trajectory.
    ...
    while changed:
        ...
        for n, o in obs_by_name.items():
            ...
            # Trajectory-free only when EVERY source is an available observation or a static network/dataset ref; a bare state/column source needs the trajectory.
...
def resolve_solver_kwargs(integration: Any, dt: float, is_diffrax: bool = False) -> str:
    ...
    if integration is None or is_diffrax:
        return ""
    kwargs = []
    # per_stage recomputes the coupling every solver stage; per_step is the native default, so only the former is emitted.
...
def render_analysis_observations(
    ...
    Covariance, PSD and Fisher all share one deterministic operating point, so the whole block — vector field, Jacobian, the operating point itself (a noise-off settle, or a constraint solve for a Deco-FIC-style tuned parameter) and each observable's linear algebra — comes out of a single Mako orchestrator, ``lr_analysis_block``. The template owns both the structure and its orchestration: partial choice, ordering, the per-observable loop. Python only resolves the spec, in ``_lr_analysis_spec``, and never string-emits a code body.
    """
    ...
    for name, aobs in analysis_obs.items():
        ...
        if atype == "lyapunov":
            ...
            # Evaluate at the current operating point: sync `wrt` (e.g. the swept coupling) into the segment config so a K-sweep reports lambda_1(K), not at the network's fixed parameters. Optional — omit `wrt` for a static Lyapunov.
            if access:
...
def render_recorded_observable(
    ...
    # A declared observation is a recorded name that is neither an analysis diagnostic nor a raw model channel; only those need compute_all_observations.
...
def get_state_bounds(model: Any) -> Tuple[List, List, bool]:
    ...
    Uses ``sympy.oo`` for unbounded dimensions so code printers render the right backend literal — ``jnp.inf``, ``np.inf``, ``Inf`` — on their own.

    A ``domain`` constrains integration only where its ``enforce`` attribute opts in. ``enforce: clamp`` hard-clips to ``[lo, hi]``; the default ``none`` makes the domain descriptive metadata — an expected or plot range, an optimisation hint, IC-sampling support — that never alters the dynamics, so a phase with domain ``[0, 2π]`` and no enforcement is left unclamped. The legacy ``boundaries`` slot is folded into ``domain`` with ``enforce: clamp`` by the Dynamics loader.
    ...
    def _finite(b):
        ...
        Range.lo/hi may also be an argument-name string or a sympy symbol (the schema permits both); those are treated as unbounded.
        """
        try:
        ...
        if domain:
            ...
            if hi is not None:
                try:
                    ...
                    pass
        # Optional optimizer start value (overrides the referenced Parameter's value), so the descent can begin from a declared point without mutating the base config.
...
def parse_exploration(expl: Any, all_couplings: Dict, get_pipeline_output_key_fn=None) -> Dict:
    ...
    exp_info = {
        ...
        "axes": [],
        # Observations computed and stacked per grid point: the declarative alternative to one scalar observable.
    ...
    for axis in axes_list:
        ...
        # ExplorationAxis.reduce: collapse this axis by a statistic in the result container instead of keeping it as a grid dim (statistic defaults to mean).
...
# Ergonomic shortcuts for the canonical `network.edges.<label>`, resolved via Network.matrix().
...
# The live backend-graph leaf each sweepable attribute rides on, so a `network.`-scoped axis needs no graph rebuild.
...
def collect_network_edge_arrays(experiment: Any) -> Dict[str, list]:
    ...
    def add(val: Any) -> None:
        ...
        # Only fully-qualified `network.*` references embed a matrix; a bare `weight` would be a state variable, not the connectome.
...
# Per-node analogue of the edge matrices: centroids and in-strength, embedded once as module constants.
