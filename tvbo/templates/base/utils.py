...
def has_distributions(model):
    ...
    return bool(collect_sv_distributions(model)
                or collect_param_distributions(model))


def get_distribution_seed(model, default=42):
    """Find the first explicit seed from any distribution, or return default."""
    for _, _, d in (collect_sv_distributions(model)
                    + collect_param_distributions(model)):
...
def graph_generator_call(gen, n_nodes, backend='julia'):
    ...
    if not mapping:
        raise ValueError(
            f"Unknown graph generator type '{gtype}'. "
            f"Known types: {list(_GRAPH_MAP.keys())}"
        )
