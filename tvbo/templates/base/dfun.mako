...
<%!
import textwrap
from tvbo.templates.base.utils import get_coupling_terms, get_func_name, get_func_args, gathered_state_indices, np_module, needs_scipy_special, model_expressions, referenced
%>
...
% endif
% if coupling_as_argument:
## The runtime's coupling vector is indexed by state-variable position, so each term unpacks at the index of the state it gathers.
% for i, ct in enumerate(global_terms):
    ${ct} = coupling[${gsi[i]}]
% endfor
% endif
