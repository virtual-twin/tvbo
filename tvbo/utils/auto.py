...
def check_auto_dir() -> str:
    ...
    if not auto_dir:
        raise EnvironmentError(
            "AUTO_DIR is not set. Export it to your auto-07p installation, e.g. "
            "`export AUTO_DIR=$HOME/Applications/auto-07p`."
        )
    auto_dir = os.path.abspath(os.path.expandvars(os.path.expanduser(auto_dir)))
    if not os.path.isdir(auto_dir):
        raise FileNotFoundError(
            f"AUTO_DIR={auto_dir!r} does not point to an existing directory."
        )
