...
def _lift_one_distribution(obj: dict) -> None:
    ...
    A ``Distribution`` carries its support under ``domain``; a bare ``lo``/``hi``/``step``
    on any ``*distribution`` slot is lifted into ``domain``, and the distribution ``name``
    is materialised as ``Uniform`` so the lifted form is a complete, valid
    ``{name: Uniform, domain: {lo, hi}}``. Any other keys (seed, axis, …) are preserved; a
    value that already states a ``domain`` is left untouched.
...
def _fold_one_state_variable_domain(sv: dict) -> None:
    ...
    * ``boundaries`` (deprecated hard-clamp slot) → ``domain`` with ``enforce: clamp``;
    ...
    the class carries no ``boundaries`` slot, so a value this cannot read would be popped
    and lost without a word.
    """
    if "range" in sv:
...
def _as_bounds_mapping(bounds: Any) -> dict:
    ...
    Accepts the mapping the YAML path always hands over, the ``(lo, hi[, step])`` sequence
    and the ``Range`` object the Python construction path may, and raises on anything else
    rather than dropping a hard clamp silently. Only the bounds are read off an object —
    ``boundaries`` means clamp, so the caller supplies ``enforce``.
    """
    bound_keys = ("lo", "hi", "step")
    if isinstance(bounds, dict):
        return dict(bounds)
    if isinstance(bounds, (list, tuple)) and 2 <= len(bounds) <= 3:
        return dict(zip(bound_keys, bounds))
