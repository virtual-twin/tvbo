...
def _vendored():
    ...
    Generated by `scripts/ontology/gen_units.py` beside the mergeable `ontology/tvb-o-units.ttl`, and read here as plain JSON so nothing on the hot path needs rdflib.
    """
...
def _field(scope, name):
    ...
    The multi-scale lowering works on the parsed dict before it is loaded into the datamodel, and asks the same question about the same slot.
    """
...
def _tool(format_key):
    ...
    Cached because the two lookups below are called once per parameter per rendered model, and the answer is a database file that does not change within a process.
    """
