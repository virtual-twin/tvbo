...
Behaviour is attached to the generated Pydantic classes as a plain mixin base rather than through a runtime subclass, and the state that behaviour needs — caches, resolved handles — rides on private attributes. That works only if four things hold of the generated models, none of which this project controls: they belong to Pydantic and to LinkML's generator.

They are pinned here because each one, if it silently stopped holding, would not fail loudly. A cache that starts serializing corrupts the specification it is written back into; a mixin that stops resolving takes the whole public API (``d.symbolic``, ``d.plot()``) with it.
"""
...
def test_a_cache_rides_on_a_private_attribute():
    ...
    This is what makes ``extra="allow"`` unnecessary: relaxing it was justified as the way to hang runtime state off a spec, and the strict config already allows exactly that, without opening the spec to stray fields.
    """
...
def test_a_spec_still_refuses_an_undeclared_slot():
    ...
    A typo'd slot in an authored recipe has to fail loudly; that is the whole reason the generated config forbids extras.
    """
...
def test_a_plain_mixin_supplies_behaviour_to_a_generated_model():
    ...
    Plain rather than a ``BaseModel`` subclass on purpose: two models in the bases would merge their ``model_config`` left to right, so a mixin carrying one could silently re-open ``extra`` or change validation for every class it is attached to.
    """
...
def test_behaviour_reaches_both_generated_forms():
    ...
    LinkML's loader still builds dataclasses while Pydantic validation builds models, so a mixin attached to only one of them would leave half the object graph without its helpers — which is what the retyping and ``setattr`` patching used to paper over.
    """
...
def test_the_schema_states_no_python():
    ...
    The schema is also what the OWL export is generated from, so an import path here would ship as a fact about the model to consumers that have no Python.
    """
