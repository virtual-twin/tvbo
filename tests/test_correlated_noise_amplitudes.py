...
`Noise.covariance` states correlation and `sigma` states amplitude, so the realised increment covariance is ``diag(sigma) C diag(sigma)``. Composing them the other way round
(``L diag(sigma)``, i.e. scaling the draw before mixing) agrees exactly when sigma is uniform along the mixed axis and silently diverges when it is not — and for a
rank-deficient covariance it does not merely distort the process, it removes it: the surviving eigenvector is placed by the eigendecomposition, and multiplying by a sigma that
...
def test_uniform_sigma_is_unchanged_by_the_fold():
    ...
    With sigma uniform along the mixed axis the two orders are bitwise identical, so the wave-model path (`correlated_over: node`, scalar sigma) is safe by construction rather
