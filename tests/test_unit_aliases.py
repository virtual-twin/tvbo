...
A row is not a preference — it asserts that two spellings are the *same quantity* — so each one is checkable against the vendored facts, and six rows were not true:
they dropped a milli prefix from a denominator, so `m/ms` was recorded as `m/s` (×1000), `kg/ms²` as `N/m` (×10⁶), and `mM` as `mmol/m³` (×1000).

Silent is the operative word. A rejected unit is a visible error, but a unit quietly rewritten to a neighbouring one leaves a model that loads, validates, renders and runs — a thousand times off.
"""
...
def compose(alias):
    ...
    Deliberately independent of `normalize_unit`: asking the alias table to check itself would confirm whatever it already says.
    """
...
def test_the_check_still_reaches_the_rows_it_used_to():
    ...
    A row becomes uncheckable when one of its atoms is not curated on its own — `uA/cm2` cannot be composed because `uA` is not a unit in its own right — so this number moves when the vocabulary does, and never quietly downwards.
    """
...
def test_millimolar_is_mole_per_cubic_metre():
    ...
    One mM is one mmol per litre and a litre is 10⁻³ m³, so the two prefixes cancel. NeuroML says the same in `NeuroMLCoreDimensions.xml`, giving `mM` and `mol_per_m3` the same power — cited as a second, independent source, since the table under test is not evidence for itself.
    """
...
def test_every_class_declaring_the_open_slot_canonicalizes_it():
    ...
    So a class that gains the slot later is covered without anyone remembering, and — the reason this is asserted — a change in the shape of those annotations fails here instead of silently switching canonicalization off everywhere.
    """
...
def test_a_real_but_uncurated_unit_is_not_rounded_to_a_curated_neighbour():
    ...
    Answering `None` sends it down the open-range path, where it is recorded as written and reported underdetermined. That is the whole reason the range was opened: the alternative on offer was never "curated or rejected", it was "curated or silently changed".
    """
