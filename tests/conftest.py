...
# ``exist_ok``) on import — from TVB_USER_HOME. Under xdist, parallel workers importing tvb race on that mkdir and the loser raises FileExistsError; give each worker its own home so the paths never collide.
...
def _isolate_tvbo_logging():
    ...
    ``propagate = False`` so the CLI owns its output and does not double-print through a host application's root logger. That is right for a CLI and wrong to leave behind in a
    test process: it is global and sticky, so once any test invokes the CLI, every later
    ``caplog`` assertion reads empty — caplog's handler sits on the root logger, which the records no longer reach — and the captured stream it kept is closed by then, so the
...
def unwrapped():
    ...
    Generated Python is black-formatted, so a long statement is split across lines at a column black chooses. Asserting on the statement's text rather than on its layout keeps
    a codegen test about what the emitter produces, not about how it was wrapped. A fixture rather than an importable helper because ``tests/`` is not a package.
    """
