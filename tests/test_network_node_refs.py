...
@pytest.mark.parametrize(
    "ref", ["network.weight", "weight", "network.edges.length", "network.observations.Bold", "theta", "positions.x", 42, None]
)
...
def test_collect_from_source_pipeline_and_observer_params():
    ...
    obs_observer = NS(source=["theta"], pipeline=[], dynamics=NS(parameters={"instr": NS(source="network.instrength")}))
    ...
    exp = SimulationExperiment(
        ...
        observations={
            "probe": {
                ...
                "pipeline": [
                    {
                        "callable": {"name": "wave", "module": "some_helper"},
                        "arguments": [
                            {"name": "theta_result", "value": "integration.result"},
