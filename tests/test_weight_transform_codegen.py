...
def test_inline_transform_is_byte_identical_to_weights_matrix():
    ...
    assert np.array_equal(_apply_emitted(net, net.raw_weights_matrix), np.asarray(net.weights_matrix))
...
def test_equation_parameters_are_substituted_like_the_runtime():
    ...
    net = Network.from_matrix(weights=W, lengths=np.zeros_like(W))
    net.transforms = [Function(name="weight", equation=Equation(rhs="weight / k", parameters={"k": {"value": 4.0}}))]
...
def test_a_third_edge_attribute_resolves_at_runtime_but_not_in_a_kit():
    ...
    Refusing at render time beats emitting a name the kit does not define and only discovering it once the job reaches a cluster.
    """
    W = np.array([[0, 2.0], [4.0, 0]])
...
def test_a_per_node_vector_is_guarded_by_node_count():
    ...
    A cohort kit is rendered once from one subject, so an unguarded vector reaches every other subject's connectome at the wrong length.
    """
    from tvbo.datamodel.schema import Node

    net = Network.from_matrix(weights=np.zeros((3, 3)), lengths=np.zeros((3, 3)))
    net.nodes = [Node(id=i, label=f"n{i}", parameters={"roi_size": {"value": float(i + 1)}}) for i in range(2)]
...
def test_every_mask_spelling_scopes_the_reduction_at_runtime(spelling):
    ...
    The whole point: a sparse connectome normalised by the all-entry mean is scaled by roughly 1/density — ~5x on a 20%-dense matrix — and nothing reports it.
    """
...
def test_both_spellings_lower_to_the_same_source():
    ...
    rendered = {
        name: weight_transform_codegen(Network.from_matrix(SPARSE, transforms=[{"name": "weight", **spec}]))[0][0][0]
...
def test_two_reductions_carry_their_own_predicates():
    ...
    net = Network.from_matrix(
        SPARSE,
        transforms=[{"name": "weight", "equation": {"rhs": "mean(weight[weight > 0]) / max(weight, weight < 4)"}}],
    )
...
def test_a_masked_extremum_fills_with_its_identity():
    ...
    empty = Network.from_matrix(SPARSE, transforms=[{"name": "weight", "equation": {"rhs": "max(weight[weight > 1e9])"}}])
...
def test_a_boolean_subscript_outside_a_reduction_is_refused():
    ...
    Inside a reduction it does — which is why the subscript spelling is accepted there and only there.
    """
    net = Network.from_matrix(
        np.array([[0.0, 2], [3, 0]]),
