...
`parameter_keypath` classifies a reference still held as raw dotted text; `axis_keypath` reads an exploration axis whose scope the classifier already resolved into flags. They are reached from different call paths — a fitted free parameter, an inference prior and a working-point ramp go through the first, the grid binding and the adiabatic sweep through the second — so a divergence between them is silent by construction: the reference lands on a same-named leaf in the wrong sub-object, or on a leaf nothing reads, and the run completes with a plausible number.

A row-by-row table cannot catch the way this actually decays, which is a seventh scope added to one resolver and forgotten in the other: the missing scope simply isn't in the table, so the table still passes. `test_every_reserved_scope_is_wired_on_both_sides` therefore derives the scopes from the module's own constants and fails on the omission rather than on a row.
"""
...
def test_every_reserved_scope_is_wired_on_both_sides():
    ...
    The module names its reserved scopes as constants, so the set is discoverable. Anything in it that this file does not exercise is a scope whose two spellings have never been compared.
    """
    declared = {v for k, v in vars(U).items() if k.startswith("_") and k.endswith("_SCOPE") and isinstance(v, str)}
...
def test_the_coupling_scope_agrees_when_the_experiment_declares_it():
    ...
    `parameter_keypath` therefore needs the declared set, while `axis_keypath` reads a flag the classifier already set — the one place the two take genuinely different inputs.
    """
...
def test_every_sweepable_edge_attribute_agrees(ref, leaf):
    ...
    `delay` is the newest and the reason this is parametrised rather than written once: the raw path picks it up through `network_axis_leaf`, so a leaf added to that table reaches `parameter_keypath` for free while `axis_keypath` needs the classifier to set `graph_leaf`.
    """
...
def test_a_flag_only_axis_resolves_without_any_declared_text():
    ...
    Delegating it to `parameter_keypath` was tried and reverted for exactly this: an axis dict carrying only flags has no `label` to read, so the text-based path returned `dynamics.sigma` for a noise axis — re-deriving a scope that was already known, and getting it wrong when the text is absent, which is the failure the shared resolver exists to remove.
    """
