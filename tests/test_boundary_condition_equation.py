...
`BoundaryCondition` held its value in a slot called `value` whose range is `Equation` — the one spelling that already means "a number" on `Parameter`. The FEM template then read it under `righthandside`, the alias the dialect folds away at construction, so `getattr` returned the `Equation` object itself, `float()` of it raised, and a bare `except` turned every Dirichlet boundary into 0.0. Nothing in the emitted code said so.
"""
...
def test_every_spelling_of_the_value_reaches_the_same_equation(written):
    ...
    The bare-scalar forms are the ones that regressed: the dialect lifted shortcuts before it folded aliases, so a value written under `value` was still under a name the shortcut table — keyed by canonical slot — could not see, and reached `__post_init__` as a str.
    """
...
def _dirichlet(code):
    """The generated ``DIRICHLET`` constraint list, as it appears in the source.

    The plan is emitted as one row per constrained state variable rather than a single module-level constant, because a field may hold several variables and each carries its own region.
    """
    return next(line for line in code.splitlines() if line.startswith("DIRICHLET"))
...
def test_a_nonzero_dirichlet_value_survives_into_the_generated_code(tmp_path):
    ...
    assert '"value": 2.5' in _dirichlet(code)
...
def test_a_zero_dirichlet_value_is_still_zero(tmp_path):
    ...
    assert '"value": 0.0' in _dirichlet(code)
...
def test_a_boundary_held_at_a_declared_parameter_resolves_to_its_value(tmp_path):
    ...
    assert '"value": 1.0' in _dirichlet(code)
