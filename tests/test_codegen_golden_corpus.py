...
def _model_paths() -> list[Path]:
    ...
    One spelling, ``.yaml``, which is also the only one `tvbo.data.registry` resolves —
    so a model this corpus renders is a model `Dynamics.from_db` can reach.
    """
...
def test_rendering_does_not_modify_the_model(fmt: str):
    ...
    Guards the property every other test here depends on. A renderer that normalises what
    it reads emits a file that depends on how many times it has been called — and the
    symbolic layer's cache, which keys on the model's content, is unsound the moment
    reading can change it.
    """
    for path in _model_paths():
...
def test_every_model_reports_without_modifying_itself():
    ...
    Two properties in one pass because they were found together. Reporting mutated the
    model exactly as rendering once did; and ten models could not produce a report at all,
    because APA formatting took a first initial from every author and BibTeX's
    `and others` idiom parses as an author with no first name.
