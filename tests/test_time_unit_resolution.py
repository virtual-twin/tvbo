...
TVBO used to answer "what unit is this duration in?" separately everywhere it was
asked. The `time_scale` slot carried `ifabsent: ms`, so it was never unset — which
made an inherited unit impossible to express and masked an explicitly declared `s`
one scope out — and six call sites in the NeuroML adapter re-derived it anyway, with
fallbacks that had drifted apart: one emitter defaulted to `s` where the rest
defaulted to `ms`, and `SimulationExperiment.to_lems` appended `ms` unconditionally
without reading the declaration at all. The database contains a model declaring `s`,
so those disagreed on real data.
...
def test_an_undeclared_inner_scope_inherits_from_an_outer_one():
    ...
    With a default on the slot the inner scope always answered `ms`, so an outer `s`
    could never be inherited — a 1000x error in every derived rate, and silent.
    """
...
def test_cross_scale_conversion_is_exact(source: str, target: str, expected):
    ...
    Exactly: the factor is a `Fraction`, because a factor of 1000 living only in a
    modeller's head is invisible in every backend's output, and one that arrives as
    999.9999999999999 is worse than none.
    """
...
def test_a_scale_that_needs_no_conversion_is_exactly_one():
    """Every network in the database declares `ms`, so this is today's only case —
    and it must be the identity, or the codegen corpus would move."""
...
def test_the_database_still_declares_one_non_default_scale():
    ...
    It is the reason this resolver exists rather than a constant, so if it ever
    disappears from the database this test should be deleted deliberately, not
    silently satisfied by a fallback.
    """
    import yaml
