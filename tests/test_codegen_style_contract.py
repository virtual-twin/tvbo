...
TVBO hands users generated code to read, review and attach to papers, so it is held to the same bar as the rest of the package. This module states that bar as an executable contract and checks every backend against it:
...
``F401`` and ``F841`` are load-bearing rather than cosmetic. Statements with no effect reach generated code when a template emits something the spec did not ask for: a parameter unpack for a coupling that has no parameters, an import a partial carries whether or not its feature is used, a state binding the pre-expression never reads. Requiring the emitted module to be free of them forces the emission to be
...
This suite freezes the CONTRACT, not the bytes. The complementary corpora that freeze emitted code and simulation output live in ``test_codegen_golden_corpus.py`` and ``test_numerical_golden_corpus.py``. The three fail on disjoint classes of regression: a spec can render byte-identically and still violate the contract only if the contract changed, but a repair that cleans up emitted code changes the bytes without touching the contract.

:data:`KNOWN_VIOLATIONS` records the pairs that do not meet the contract yet, each mapped to the exact rule codes it still trips. The record is pinned from both sides — a code that appears without being recorded fails, and a recorded code that stops appearing fails too — so repairing an emitter reports as a failure asking for the entry to be updated, the same reconciliation the golden corpora use for their unrenderable pairs. A silently skipped backend cannot be told from one nobody noticed.
"""
...
def test_generated_python_is_lint_clean(name, fmt):
    ...
    Pairs listed in :data:`KNOWN_VIOLATIONS` are allowed to trip exactly the codes recorded for them, and no others — a NEW violation in a known-bad pair still fails.
    """
...
def test_known_violations_are_reconciled(case):
    ...
    Asserting only that SOME recorded code still trips would let a partially repaired entry go stale — which is how ``F821`` stayed listed for tvboptim after the undefined names were fixed. Every recorded code must still be observed.
    """
...
FROZEN_PYTHON = sorted(
    path
    for path in (Path(__file__).parent / "reference_data" / "codegen").glob("*.txt")
    if path.stem.rsplit(".", 1)[-1] in {"jax", "numpy", "tvboptim"}
)


@pytest.mark.parametrize("path", FROZEN_PYTHON, ids=[p.stem for p in FROZEN_PYTHON])
def test_frozen_generated_python_binds_every_name_it_reads(path: Path):
    ...
    assert "F821" not in codes, f"{path.stem} reads a name nothing binds"
