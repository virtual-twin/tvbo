...
A cortical mesh is a 2-manifold carrying three coordinates, which the general-purpose FEM
libraries do not assemble on — scikit-fem builds the mesh and then divides by zero inside
its affine mapping. So the operators are written in closed form, and these tests pin them
against the two references that can settle it: scikit-fem itself wherever the two agree
(a flat mesh), and the analytic Laplace-Beltrami spectrum of a sphere where only the
closed form applies.
"""
...
def test_operators_do_not_change_when_the_mesh_is_moved_through_space(flat):
    """The same triangles rotated out of the plane and translated: a surface operator
    depends on the metric, not on the coordinates it happens to be written in.

    This is the concrete regression. Handed a cortical surface, scikit-fem does not raise —
    it assembles the mesh's shadow on the xy-plane, and the mass matrix it returns totals the
    projected area rather than the surface area (33,070 mm2 against the true 69,589 mm2 on
    fsLR-32k). A rotation invariance check is what turns that from a plausible animation into
    a failing test.
    """
...
def test_the_sphere_spectrum_converges_to_the_analytic_one(icosphere):
    ...
    The eigenvalue test is the one that would catch a plausible-but-wrong operator: a
    stiffness matrix can be symmetric, positive semi-definite and have the right nullspace
...
def test_a_closed_surface_has_no_boundary_and_carries_its_whole_area(icosphere):
    """The mass matrix's total is the surface area, and the stiffness annihilates
    constants — the discrete statement that no flux leaves a closed surface, which is why
    a Dirichlet condition declared on a cortical mesh constrains nothing."""
...
def test_a_constant_coefficient_just_scales_the_operator(icosphere):
    """The gate on the weighted assembly: where the weighted and plain forms describe the
    same operator they must be the same matrix, not merely a similar one."""
...
def test_a_varying_coefficient_integrates_exactly(icosphere):
    """Both weighted forms are exact for a P1 coefficient rather than a one-point rule, so
    each has an integral identity that pins it: ``1.M(c).1`` is the integral of ``c``, and
    ``M(c)`` stays symmetric positive-definite."""
    vertices, faces = icosphere(3, 2.0)
...
def test_a_degenerate_triangle_is_refused(icosphere):
    """A zero-area element has no P1 gradient. Saying so beats dividing by zero and
    returning an operator full of infinities."""
