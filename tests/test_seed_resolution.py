...
so a distribution's own ``seed`` is a *local* override of the experiment-global ``execution.random_seed``, and both the single-trial and the ``n_trials`` initial-condition paths resolve it identically. The ``n_trials`` sampler also keys each trial off ``fold_in(base, i)`` so a trial's IC is independent of ``n_trials`` (adding a trial never reshuffles the existing ones).
"""
...
def test_each_distributed_variable_uses_its_own_seed():
    """With two distributed state variables carrying distinct seeds, BOTH seeds appear — the ensemble keys each variable off its own seed, not only the first (regression for the n_trials sampler that used a single base key)."""
    spec = copy.deepcopy(BASE)
