...
A paper labels 47 task contrasts as seven families — one name per family, centred on its
block, with a rule between blocks — not as 47 tick labels. The same shape recurs wherever a
categorical axis has structure (ROIs by system, nodes by module, subjects by cohort), so it
is an axis feature rather than something each bespoke composite panel redraws.

The convention worth pinning is what a `bound` IS. They are cumulative COUNTS, so the group
sizes are readable straight off the declaration; an entry is drawn centred on its index, so
the gap after count n sits at n - 0.5. Declaring plotted coordinates instead would put the
half-cell shift in every recipe and get it wrong in some of them.
"""
...
def test_a_label_sits_at_the_centre_of_its_block():
    ...
    assert [l["at"] for l in g["labels"]] == [1.0, 9.0, 17.0]
    assert [l["text"] for l in g["labels"]] == LABELS
...
def test_bounds_are_counts_so_a_group_of_three_spans_indices_zero_to_two():
    ...
    assert g["rules"] == [2.5]
    assert _group_axis({"ygroups": {"bounds": [3, 6], "labels": ["a", "b"]}},
                       "y")["labels"][0]["at"] == 1.0
...
def test_both_axes_can_carry_groups_at_once():
    ...
    panel = _Panel(kind="heatmap", opts={"ygroups": {"bounds": BOUNDS},
                                         "xgroups": {"bounds": [2, 4]}})
