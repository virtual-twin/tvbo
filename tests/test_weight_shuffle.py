...
A masked extract, a permutation and a scatter are not expressible in the backend-independent primitive set: ``M[M != 0]`` cannot survive expression parsing at all, because the comparison evaluates to a plain Python ``True`` before a tree is built. Rather than invent a contorted primitive that only this generator would use, the algorithm stays as Python — the documented exception to the typed-DAG rule, kept for exactly this case.
...
def test_an_unimplemented_preserve_mode_is_rejected(monkeypatch):
    ...
    `degree` and `weight_distribution` are documented in the generator's parameters but have no implementation; a caller asking for a degree-preserving null must not be handed a binary-mask one.
    """
