...
The tables this replaced sat in `tvbo.utils.units` as though "what LEMS calls a
`per_time` quantity" were a fact about the unit. It is a fact about LEMS, and it is
the reason a second backend could not state its own without editing a module every
...
def test_an_unclaimed_format_is_none_not_an_error():
    """Most formats have no tool entry, and the caller's fallback is the same as
    for a tool that is merely silent on a unit."""
...
def test_a_tools_spelling_can_differ_from_the_unit_it_translates(lems):
    """`Hz` and `kHz` are written `per_s` and `per_ms` — which is exactly why the
    vocabulary cannot be derived from the unit and has to be declared."""
...
def test_a_unit_the_tool_does_not_know_is_dimensionless_and_bare(lems):
    ...
    It has to come back `("none", "")` rather than raise: an emitter asking how to
    write a value needs an answer for every value.
    """
...
def test_only_units_the_tool_names_are_recorded(lems):
    ...
    24 of the 54 entries in the tables this replaced said `("none", "")` — the
    default spelled out — so they claimed nothing and are gone.
    """
...
def test_a_tool_may_name_units_outside_the_curated_vocabulary(lems):
    ...
    They are kept because the `unit` slot's range is open (U14): a model may
    declare `uA`, and LEMS knows how to write it even though TVBO holds no QUDT
    record for it. Dropping them would silently downgrade such a model to unitless.
    """
...
def test_physical_dimension_is_gone():
    ...
    Nothing declared a slot of that range — it was reachable only by importing it —
    so the names now live on the one tool that actually uses them.
    """
