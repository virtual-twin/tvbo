...
def test_sample_exponential_shape_support_and_mean():
    src = render_expression("sample_exponential(key, 17.0, n, n)", format="jax")
...
def test_sample_normal_moments():
    src = render_expression("sample_normal(key, 3.0, 2.0, n, n)", format="jax")
...
def test_sample_uniform_support():
    src = render_expression("sample_uniform(key, -1.0, 1.0, n, n)", format="jax")
...
def test_connection_mask_composes_from_primitives():
    ...
    """
    expr = "d <= abs(sample_exponential(key, 17.0, n, n))"
