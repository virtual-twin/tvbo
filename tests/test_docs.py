    ...
    try:
        ...
        setup_cell = {
            ...
            "source": [
                ...
                "        # Surface f2py errors that PyRates suppresses\n",
                "        fname = kwargs.get('file_name', 'pyrates_func')\n",
                "        f90 = fname + '.f90'\n",
                "        if os.path.exists(f90):\n",
                "            r = _sp.run(f'python -m numpy.f2py -c -m {fname} {f90}',\n",
                "                        shell=True, capture_output=True, text=True, cwd=_cwd)\n",
                "            if r.returncode != 0:\n",
