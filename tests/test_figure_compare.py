...
The verb exists so "the figure is the wrong shape" becomes a number instead of an impression, which only works if the decomposition is trustworthy: it must find the panels a reader would point at, pair them with the reference's, and not invent structure where there is none.
"""
