...
def test_every_named_template_exists(record):
    ...
    for rel, entry in layout_rules.iter_files(record, layout_rules.ANY_TEMPLATE):
...
def test_every_template_an_entry_names_is_one_the_record_declares(record):
    ...
    for rel, entry in entries:
        ...
        assert named <= declared, f"{rel} names {sorted(named - declared)}, which `templates:` does not declare"


def test_a_template_name_the_record_does_not_declare_is_rejected(record):
    """Matched, never registered: without this the scaffolder accepts any word and silently ignores it."""
    assert layout_rules.check_templates(("replication",), record) == ("replication",)
    with pytest.raises(KeyError, match="replicaton"):
        layout_rules.check_templates(("replicaton",), record)


def test_a_collection_gets_the_document_in_place_of_a_report(record):
    """`not_in_templates` is what lets one kind replace an entry every other kind has."""
    collection = dict(layout_rules.iter_files(record, ("collection",)))
    general = dict(layout_rules.iter_files(record, ()))
    assert "docs/manuscript.qmd" in collection and "docs/report.qmd" not in collection
    assert "docs/report.qmd" in general and "docs/manuscript.qmd" not in general


def test_only_a_collection_carries_the_results_manifest(record):
    """A study that reports nothing has no manifest, so its ignore files must not claim one."""
    assert "manuscript_results.yml" in dict(layout_rules.iter_files(record, ("collection",)))
    assert "manuscript_results.yml" not in dict(layout_rules.iter_files(record, REPLICATION))


def test_a_study_declares_its_kind_even_when_it_keeps_its_own_rules(record):
    """The kind is read back from the generated header, which a project may keep below its own rules."""
    generated = "# Generated by `tvbo study init` from schema/study_layout.yaml (template: collection). Edit the record, not this file."
    assert layout_rules.templates_of(f"# this project's own rules\n*.zip\n\n{generated}\nspec/") == ("collection",)
