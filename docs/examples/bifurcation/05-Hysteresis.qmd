...
aliases:
  - /Bifurcation/05-Hysteresis.html
...
Numerically integrate the system while *slowly* sweeping $a$ first up ($-1 \to 1$) then back down ($1 \to -1$). Trajectories *track* the upper branch on the way up and the lower branch on the way down — the system
...
The system *jumps* discontinuously to the opposite branch precisely when each saddle-node is crossed. The enclosed area of the loop is the
