...
aliases:
  - /Bifurcation/02-PhasePortrait2D.html
  - /Usage/BifurcationTutorial/02-PhasePortrait2D.html
---
...
Each panel uses the new `kind='phaseplane'` shortcut, which overlays the vector field with the two **nullclines** ($\dot x_1 = 0$ in blue,
$\dot x_2 = 0$ in red) and marks the **fixed points** detected on the grid (black circles). Axis limits are set declaratively via the YAML `domain.lo / hi`.
