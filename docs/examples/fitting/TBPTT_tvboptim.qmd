...
aliases:
  - /Fitting/TBPTT_tvboptim.html
  - /Interoperability/tvboptim/TBPTT_tvboptim.html
---

Why the exact FC-loss gradient breaks past the chaos onset, and how a single `integration.differentiation` window fixes it. The adiabatic bifurcation envelope, the diagnostics swept over the global coupling $G$ (loss, autodiff gradient, finite-difference ground truth, top Lyapunov exponent) and both optimization legs all derive from **one** experiment YAML via `exp.run`; only the plotting is hand-written. The figures reproduce the original tvboptim [`TBPTT.qmd`](https://github.com/virtual-twin/tvboptim/blob/main/docs/workflows/TBPTT.qmd) workflow through the tvbo declarative pipeline.

The sweep spreads its points over as many CPU devices as `TVBO_XLA_DEVICE_COUNT` names. The motivation sweep's reverse-mode autodiff diagnostic keeps the whole 60 s forward trajectory on tape, so one point already peaks near 13 GB and every extra device multiplies that; one device (the default) builds the page within a 32 GB machine with an identical result, only slower, and the reference uses 4 on a larger host. Eigen's intra-op threading is off because the devices share one XLA thread pool, in which multithreaded FFTs running on several devices at once can deadlock.

```{python}
import os

device_count = os.environ.get("TVBO_XLA_DEVICE_COUNT", "1")  # the sweep's parallel CPU devices; see above
os.environ["XLA_FLAGS"] = f"--xla_force_host_platform_device_count={device_count} --xla_cpu_multi_thread_eigen=false"
