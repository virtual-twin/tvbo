...
aliases:
  - /Fitting/Delay_Speed_Synchronization_tvboptim.html
  - /Interoperability/tvboptim/Delay_Speed_Synchronization_tvboptim.html
---

Transmission delays organize the synchronization of a brain network of coupled phase oscillators. Because the connectome carries **tract lengths**, tvbo lowers the delayed coupling onto a `DenseLengthGraph`: the backend derives `delays = lengths / conduction_speed` on every forward pass, so the conduction speed is a *live graph leaf*: the delay-domain twin of the coupling gain $G$.
Sweeping it varies the delays and traces out the non-monotonic synchronization resonance of Petkoski & Jirsa (2019). The exploration is declared entirely in the [`Delay_Speed_Synchronization`](https://github.com/virtual-twin/tvbo) experiment;
here we load it, run it, and confirm the generated code is **byte-identical** to a hand-written native tvboptim workflow.
...
os.environ["XLA_FLAGS"] = f"--xla_force_host_platform_device_count={device_count} --xla_cpu_multi_thread_eigen=false"  # the devices share one XLA thread pool, where threaded FFTs can deadlock
