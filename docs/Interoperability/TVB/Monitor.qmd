...
coupling, integrator, and initial conditions.
...
np.random.seed(seed)
sim1.configure()
y1 = np.concatenate([r[0][1] for r in sim1(simulation_length=sim_length) if r[0] is not None])
...
np.random.seed(seed)
sim2.configure()
y2 = np.concatenate([r[0][1] for r in sim2(simulation_length=sim_length) if r[0] is not None])

print(f"TVB native cvar:    {sim1.model.cvar}")
print(f"tvbo-generated cvar: {sim2.model.cvar}")
print(f"Output shape: {y1.shape}")
print(f"Max absolute diff:  {np.abs(y1 - y2).max():.2e}")
print(f"Numerically equal:  {np.allclose(y1, y2, atol=1e-10)}")
```
...
node = 0
t = np.arange(y1.shape[0]) * 0.1

fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 4), sharex=True, height_ratios=[3, 1])

ax1.plot(t, y1[:, 0, node], label="TVB native", lw=1.2)
ax1.plot(t, y2[:, 0, node], label="tvbo-generated", lw=1.2, ls="--")
ax1.set_ylabel("V")
ax1.legend()
ax1.set_title(f"Region {node}: {conn.region_labels[node]}")

ax2.semilogy(t, np.abs(y1[:, 0, node] - y2[:, 0, node]) + 1e-20, color="k", lw=0.8)
ax2.set_ylabel("|diff|")
