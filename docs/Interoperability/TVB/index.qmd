...
Convert a fully configured TVB `Simulator` into a tvbo
`SimulationExperiment` — preserving the model, coupling, integrator,
monitors, noise, and network:
...
print(f"Coupling:   {', '.join(exp.network.coupling)}")
...
The imported experiment is a standard tvbo object — you can inspect
the YAML representation, modify parameters, or re-export:
...
Any tvbo `Dynamics` can be compiled into a TVB-compatible `Model`
object. This uses Mako code generation to produce a TVB Model subclass:
...
A `SimulationExperiment` can also be defined from a YAML string —
useful for sharing reproducible experiment specifications:
...
observations:
  Raw:
    name: Raw
""")
# The connectome comes from TVB; the declared coupling rides along on it
tvb_network = Network.from_tvb(conn)
for name in exp2.network.coupling:
    tvb_network.coupling[name] = exp2.network.coupling[name]
exp2.network = tvb_network
...
For connectome-only workflows, import TVB connectivity without a full
simulator. See the [Connectome Round-Trip](Connectome.qmd) page for
the complete walkthrough including surface meshes and HDF5 persistence.
