...
All TVB per-node metadata is preserved — cortical flags, areas, hemispheres, and orientations:
...
triangles = surface_net.array("mesh/elements")
...
print(f"Mesh vertices: {reloaded_surf.array('mesh/vertices').shape}")
print(f"Mesh triangles: {reloaded_surf.array('mesh/elements').shape}")

np.testing.assert_allclose(reloaded_surf.array("mesh/vertices"), vertices)
np.testing.assert_allclose(reloaded_surf.array("mesh/elements"), triangles)
print("✓ Mesh data survives HDF5 round-trip")
```

### Optional: MNE Projection Layer in the Network Section

The MNE exporter creates a sensor-to-region projection network in tvbo format (`sensors_eeg_standard1005_fsaverage_aparc_projection.yaml`).
This can be handled in the same multi-layer network workflow as other network resources.
