...
::: {.callout-tip}
The distribution-based IC and parameter sampling described above is specific to the NetworkDynamics.jl (Julia) backend. For the Python/tvboptim backend, see [Initial Conditions & Initial State](../../Models/InitialConditions.qmd) — which covers `state:` per-node overrides, `Exploration(n_trials=N)` sampling, and transient warm-up.
:::
