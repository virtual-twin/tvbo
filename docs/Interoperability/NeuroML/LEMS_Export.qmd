...
- `<TimeDerivative variable="V" value="..."/>` — note the `/ SEC` when needed to
...
**`AttributeError: 'Dynamics' object has no attribute 'to_lems'`**
: Removed at 1.0. Use `NeuroMLAdapter(model).render_code()` instead.
