...
for name, ref_arr in ref_outputs.items():
    ...
    n_show = min(ref_arr.shape[1] - 1, 10)
    for i in range(1, n_show + 1):
        ax.plot(t, ref_arr[:, i] * 1000, alpha=0.7, label=f'Cell {i-1}')
...
::: {.callout-note}
## Input Types in TVBO
Each input type maps to a NeuroML network element: current inputs (pulse, sine, ramp,
voltage clamp, compound, poissonFiringSynapse, transientPoisson, timedSynapticInput)
become `<explicitInput>` targets; spike event sources (spikeArray, spikeGenerator*) become
populations with `<synapticConnection>` to target cells via syn1.
:::
