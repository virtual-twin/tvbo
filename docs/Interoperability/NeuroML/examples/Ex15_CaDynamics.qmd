...
::: {.callout-note}
## Granule Cell Composition Tree
This cell uses the full TVBO composition tree with 7 ion channels, a decaying
calcium pool, and a calcium-dependent potassium channel.  Custom rate types
use `derived_variables` with `Piecewise(...)` for conditional expressions.
The KCa channel rates reference `caConc` (automatically detected as a LEMS
Requirement extending `baseVoltageConcDepRate`).
:::
