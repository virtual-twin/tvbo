...
::: {.callout-note}
## Network 2
Demonstrates three synapse types with weight and delay modulation:
- `expOneSynapse`: single-exponential decay
- `expTwoSynapse`: dual-exponential (rise + decay)
- `blockingPlasticSynapse` (NMDA): with Mg²⁺ voltage-dependent block via child `blockMechanism`

The NMDA synapse is defined as a dynamics entry with a composition tree:
synNmda (blockingPlasticSynapse) → blockMechanism (voltageConcDepBlockMechanism).
Same pattern as channels with gates — no lookup tables needed.
:::
