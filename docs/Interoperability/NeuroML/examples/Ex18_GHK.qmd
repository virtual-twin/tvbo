...
The calcium current uses `channelDensityGHK` with a `permeability` instead of `condDensity`/`erev`, and includes an exponentially decaying calcium pool (`fixedFactorConcentrationModel`). The Ca gate uses `q10Fixed` with a hardcoded time factor.
