...
jLEMS child-before-parent RK4 evaluation order, so the simulated traces match the
reference simulation to correlation r ≈ 1. The emitted XML is equivalent, not
byte-identical, to the canonical NeuroML file.
...
jLEMS, so the traces match the reference simulation to correlation r ≈ 1 at the original
...
The same Hodgkin-Huxley model, defined with **full equations** — no reference to
standard NeuroML component types. Uses `iri: extends:base*` to declare LEMS base
type compatibility while providing all dynamics explicitly. This makes the YAML
interoperable with any backend, not just NeuroML.
...
Every equation — rate functions, gating kinetics, channel conductance — is defined explicitly
in the YAML. The adapter generates custom LEMS `ComponentType` definitions that extend base
infrastructure types, reproducing the standard version's traces to correlation r ≈ 1.
:::
