...
3 pre-synaptic HH cells receive weighted current injection;
9 post-synaptic HH cells receive synaptic input via different projection types.
...
for name, ref_arr in ref_outputs.items():
    ...
    for i in range(1, min(ref_arr.shape[1], 10)):
...
All four NeuroML weight-bearing constructs are demonstrated:
`connectionWD`, `electricalConnectionInstanceW`, `continuousConnectionInstanceW`,
and `inputW`.  TVBO maps edge `weight` parameters to the appropriate NeuroML
format for each connection class.
:::
