...
if ref_v is not None:
    ...
    axes[0].set_title(f"Ex24: Fractional Conductance — corr={corr:.4f}, RMSE={rmse:.2f} mV")
    axes[0].legend()
...
- `gateFractional` with `subGate` — two sub-gates sharing conductance 50/50
- Custom LEMS `ComponentType` definitions built from `derived_variables`
- Standard NeuroML rate types (`HHExpLinearRate`, `HHExpRate`) coexisting with custom types
- `cell` type with morphology + `channelDensity` (vs `pointCellCondBased` + `channelPopulation`)
- Two pulse generators with opposite amplitudes
- Temperature scaling via `networkWithTemperature`
:::
