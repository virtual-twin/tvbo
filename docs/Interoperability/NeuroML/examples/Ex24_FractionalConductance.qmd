...
Demonstrates `<gateFractional>` with `<subGate>` — the slow potassium channel uses a fractional conductance formalism where gate `b` is composed of two sub-gates (`bb`,
...
Standard NeuroML types use `iri: neuroml:*` shortcuts; custom types use `derived_variables:` to build LEMS `<ComponentType>` definitions from equations.
