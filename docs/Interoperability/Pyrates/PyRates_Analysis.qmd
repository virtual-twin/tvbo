...
from tvbo.datamodel.schema import Exploration, ExplorationAxis, Range
...
exp.explorations['mu_sweep'] = Exploration(
    ...
    space=[ExplorationAxis(parameter='VdP.mu', domain=Range(lo=0.1, hi=10.0, n=6))],
    mode='product',
)
...
exp2.explorations['amp_landscape'] = Exploration(
    ...
    space=[
        ExplorationAxis(parameter='a',     domain=Range(lo=0.5, hi=1.0, n=3)),
        ExplorationAxis(parameter='I_ext', domain=Range(lo=0.0, hi=1.0, n=3)),
    ],
    mode='product',
)
...
a_name, I_name = sweep2.axes[0].name, sweep2.axes[1].name  # dotted parameter refs
a_vals   = sweep2.axes[0].explored_values
I_vals   = sweep2.axes[1].explored_values
...
```{python}
# Steady-state peak-to-peak amplitude of v per grid point, computed on the labeled
# xarray (as_grid() keys the sweep by parameter name), so no positional reshape.
_v = sweep2.as_grid().sel(variable='v')          # dims (a, I, time), keyed by name
_v = _v.isel(time=slice(n_time2 // 2, None))       # steady-state window
amps_grid = (_v.max('time') - _v.min('time')).transpose(I_name, a_name)  # rows=I, cols=a

fig, ax = plt.subplots(figsize=(7, 5))
im = ax.imshow(
    amps_grid.values, origin='lower', aspect='auto',
...
exp3.explorations['eta_sweep'] = Exploration(
    ...
    space=[ExplorationAxis(parameter='QIF.eta', domain=Range(lo=-10.0, hi=5.0, n=15))],
    mode='product',
)
...
|---|---|
| `Exploration.space` + `Range(lo, hi, n)` | `param_grid = {name: linspace(lo, hi, n)}` |
...
exp.explorations['my_sweep'] = Exploration(
    name='my_sweep',
    space=[ExplorationAxis(parameter='Model.mu', domain=Range(lo=0.1, hi=10.0, n=6))],
    mode='product',  # or 'zip'
)
