...
| Inputs | `coupling_inputs` | `input(value)` marker |
...
result = exp.run().integration

# Plot TVBO simulation
fig, axes = plt.subplots(1, 2, figsize=(12, 4))

time = result.time
x = result.data.isel(variable=0).values
z = result.data.isel(variable=1).values
...
for name, dyn in exp_multi.network.dynamics.items():
    print(f"\n=== Operator: {name} ===")
...
tsodyks = exp_multi.network.dynamics["tsodyks"]
...
result = synapse_exp.run().integration

# Plot synaptic dynamics
fig, axes = plt.subplots(1, 3, figsize=(14, 4))

time = result.time
x = result.data.isel(variable=0).values  # synaptic resources
u = result.data.isel(variable=1).values  # utilization factor
...
tvbo_result = exp_compare.run().integration
...
# Compare
tvbo_x = tvbo_result.data.isel(variable=0).values
tvbo_z = tvbo_result.data.isel(variable=1).values
