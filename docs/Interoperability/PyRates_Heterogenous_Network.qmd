...
print(f"Created dynamics: {driver.name}, {fhn.name}, {vdp.name}")
```
...
from tvbo.datamodel.tvbopydantic import Network
...
nodes:
  ...
  - id: 2
    ...
    dynamics: Relaxation
edges:
  # Driver excites the Excitable node (positive weight)
  - source: 0
    target: 1
    weight: 0.8
    source_var: x_out
    target_var: c_in
  # Driver inhibits the Relaxation node (negative weight)
  - source: 0
    target: 2
    weight: -0.6
    source_var: x_out
    target_var: c_in
  # Weak mutual coupling between nodes 1 and 2
  - source: 1
    target: 2
    weight: 0.1
    source_var: v_out
...
ax = axes[0]
ax.plot(t, result["driver"], 'C0', lw=2, label="Slow Driver")
ax.axhline(0, color='gray', ls='--', alpha=0.5)
ax.fill_between(t, result["driver"], 0, where=result["driver"] > 0,
                alpha=0.3, color='C0', label="Driver UP phase")
ax.set_ylabel("Driver\n(Hopf)")
ax.legend(loc='upper right')
ax.set_title("Heterogeneous Network: Slow Driver Modulates Two Different Dynamics", fontsize=12)

# Plot Excitable (responds to positive driver input)
ax = axes[1]
ax.plot(t, result["excitable"], 'C1', lw=1.5, label="Excitable (FHN)")
ax.axhline(0, color='gray', ls='--', alpha=0.5)
# Shade when driver is positive (excitable should be more active)
driver_up = result["driver"].values > 0
ax.fill_between(t, ax.get_ylim()[0], ax.get_ylim()[1],
                where=driver_up, alpha=0.15, color='C0')
ax.set_ylabel("Excitable\n(FHN)")
ax.legend(loc='upper right')
ax.annotate("Active when Driver UP", xy=(0.02, 0.85), xycoords='axes fraction',
            fontsize=10, color='C0')

# Plot Relaxation (responds to negative driver input - anti-phase)
ax = axes[2]
ax.plot(t, result["relaxation"], 'C2', lw=1.5, label="Relaxation (VdP)")
ax.axhline(0, color='gray', ls='--', alpha=0.5)
# Shade when driver is negative (relaxation should be more active)
driver_down = result["driver"].values < 0
ax.fill_between(t, ax.get_ylim()[0], ax.get_ylim()[1],
                where=driver_down, alpha=0.15, color='C2')
ax.set_ylabel("Relaxation\n(VdP)")
ax.set_xlabel("Time (ms)")
ax.legend(loc='upper right')
ax.annotate("Active when Driver DOWN", xy=(0.02, 0.85), xycoords='axes fraction',
...
print("Edges:")
for e in network_plasticity.edges:
