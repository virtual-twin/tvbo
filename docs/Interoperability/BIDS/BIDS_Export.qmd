...
```{python}
from tvbo.datamodel.schema import CouplingInput
...
# Define the coupling function, on the network whose edges it acts along
coupling = Coupling.from_ontology('Linear')
connectome.coupling[str(coupling.name)] = coupling
...
for i, coupling_strength in enumerate([0.01, 0.02, 0.03]):
    ...
    modified_exp.network.coupling['Linear'].parameters['a'].value = coupling_strength
