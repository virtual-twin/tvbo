...
subtitle: "Exporting a simulation through the legacy to_bids() path"
author: "TVB-O Team"
date: last-modified
format:
  html:
...
This notebook runs a small simulation and exports it through `to_bids()`, TVBO's export path for [BIDS](https://bids.neuroimaging.io/) datasets.

## What BIDS gives you here

BIDS fixes how files are named, where they sit, and how metadata travels beside them in JSON sidecars. For a simulation that means a result another tool can find and a record of the parameters that produced it, without a bespoke reader.

## A note on BEP034

BEP034 is the draft BIDS extension for computational modelling. As compiled from its pull request it defines one datatype, `model`, with `eq` and `param` suffixes in LEMS XML, and nothing else.

The layout produced below is different: `net/`, `ts/`, `eq/`, `coord/` and `map/` directories with `id-<hash>` filenames. It follows an early BEP034 draft rather than the current proposal, and TVBO's own `adapters/bep034.json` declares those names as filename entities rather than directories. Treat this export path as legacy, and see [Computational Modelling](BIDS_comp_modelling.md) for what TVBO writes from `tvbo run` instead.

Reference:

- [BIDS Specification](https://bids-specification.readthedocs.io/)
- [BEP034](https://bids.neuroimaging.io/bep034)
...
A three-node connectome and a Generic2dOscillator, linearly coupled.
...
`to_bids()` writes the simulation results as a dataset in the legacy layout described above.

## Output Formats

Time series export in one of three formats:
...
# Inspect the Exported Structure

The export splits its output across directories by content type:
