...
data_bold = bold_obs.data[:t_bold_max, 0, :]
...
# Mark fitted value
G_fit = opt_results.global_optimization.fitted_params.coupling.instant.G.value
w_fit = opt_results.global_optimization.fitted_params.dynamics.w.value
ax.scatter(
    G_fit,
    w_fit,
    color="white",
...
G_route = np.array(
    [ds.coupling.instant.G.value for ds in opt_results.global_optimization.fitting_data["state"].save]
)
w_route = np.array(
    [ds.dynamics.w.value for ds in opt_results.global_optimization.fitting_data["state"].save]
)
ax.scatter(
    G_route[::2],
    w_route[::2],
...
sim_global = exp.run("tvboptim", mode="simulation", state=opt_results.global_optimization.fitted_params)
fc_global = np.array(sim_global.observations.fc.data)

sim_regional = exp.run("tvboptim", mode="simulation", state=opt_results_het.regional_optimization.fitted_params)
fc_regional = np.array(sim_regional.observations.fc.data)
...
w_global = opt_results.global_optimization.fitted_params.dynamics.w.value
I_o_global = opt_results.global_optimization.fitted_params.dynamics.I_o  # Not optimized in global fit, but initial value
