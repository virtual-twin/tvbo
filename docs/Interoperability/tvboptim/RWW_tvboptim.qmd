...
local_dynamics:
    ...
    parameters:
        w:
            ...
            value: 0.5
            description: "Local feedback strength"
            free: true
            domain:
                lo: 0.0
                hi: 1.0
        ...
        tau_s:
            ...
            value: 100.0
            unit: ms
            description: "NMDA receptor time constant"

        gamma:
            label: "Kinetic parameter"
            value: 0.641
            description: "Saturation factor for firing rate"

        J_N:
            label: "NMDA efficacy"
            value: 0.15
            description: "Effective NMDA receptor conductance"

        a_tf:
            label: "Transfer function slope"
            value: 270.0
            unit: Hz/nA
            description: "Slope of input-output transfer function"

        b_tf:
            label: "Transfer function offset"
            value: 108.0
            unit: Hz
            description: "Offset of transfer function"

        d_tf:
            label: "Transfer function time constant"
            value: 0.154
            unit: s
            description: "Time constant of transfer function"

    derived_variables:
        x:
            label: "Total synaptic input"
            equation:
                rhs: "w * J_N * S + I_o + c_instant"
            description: "Sum of local recurrence, external input, and long-range coupling"

        H:
            label: "Population firing rate"
            equation:
                rhs: "(a_tf * x - b_tf) / (1 - exp(-d_tf * (a_tf * x - b_tf)) + 1e-10)"
...
functions:
    ...
    bold_convolve:
        description: "Convolve signal with HRF kernel"
        callable:
        ...
        arguments:
            - name: data
              description: "Signal with history prepended (time, nodes)"
            - name: hrf
              description: "HRF kernel array"
            - name: mode
              value: "valid"
    ...
    compute_fc:
        name: compute_fc
        description: "Compute functional connectivity from BOLD signal"
        callable:
            name: compute_fc
...
optimization:
    - name: fc_fitting
      ...
      loss:
          ...
          label: "FC RMSE Loss"
          equation:
              rhs: "sqrt(mean((simulated_fc - empirical_fc)**2))"
          description: "Root mean squared error between FC matrices"
...
bold_obs = sim_results.observations.simulated_bold
t_bold_max = 60
...
fc_initial = np.array(sim_results.observations.fc)
...
G_route = np.array([ds.coupling.instant.G.value for ds in opt_results.fitting_data["state"].save])
w_route = np.array([ds.dynamics.w.value for ds in opt_results.fitting_data["state"].save])
...
```{python}
opt_results_het = exp.run("tvboptim", mode="optimization", stage="regional_optimization",
