...
aliases:
  - /ScaleOut/CodegenPhilosophy.html
---
...
This is why a model written once compares across backends at all. The equation is never re-typed per backend, so there is no opportunity for two backends to disagree about what the model *is* — only about how they integrate it, which is a [separate and measurable question](../2-specify/Integrators/CrossBackendParity.qmd).
...
- [Run on a backend](../4-run/Backends.qmd): the capability matrix and how work is dispatched.
- [Cross-backend numerical parity](../2-specify/Integrators/CrossBackendParity.qmd): why two backends can still disagree, and by how much.
- [Distributed workflows](workflows/DistributedWorkflows.qmd): what the planner does with a backend's declared capabilities.
