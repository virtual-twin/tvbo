...
six static panels for fixed values of $a$. Here we use the new
`Dynamics.animate(...)` shortcut to *sweep* $a$ continuously across the
bifurcation, generating a single animation that captures the entire
qualitative transition from stable focus to limit cycle.

```{python}
from IPython.display import Image

dyn = Dynamics.from_string(HOPF_YAML)
anim = dyn.animate("a", np.linspace(-2.0, 1.5, 24),
...
Image("hopf_sweep.gif")
```

The blue and red curves are the two nullclines $\dot x_1 = 0$ and
$\dot x_2 = 0$; their intersection is the equilibrium. As $a$ crosses $0$
the equilibrium changes from a stable focus to an unstable one, and a
stable limit cycle of radius $\sqrt a$ appears around it.
