...
| 2 | [2D Phase Portraits](02-PhasePortrait2D.qmd) | `PhasePortrait2DLinearSystem.py` | $\dot{\vec x} = A\vec x$, saddle | `PhasePortraitSaddle.png` |
...
For pure phase-portrait figures we use `plot_dynamics` with `kind="vectorfield"`:

```python
from tvbo.plot.dynamics import plot_dynamics
plot_dynamics(dyn, "x1", "x2", kind="vectorfield")
```

This produces a stream-plot of $\dot{\vec x}$ over the state-variable
domains declared in the YAML.
