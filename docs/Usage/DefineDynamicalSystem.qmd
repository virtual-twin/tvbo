...
```{python}
# | echo: false
x = results.integration.sel(variable='x').values
y = results.integration.sel(variable='y').values
...
ax_timeseries = axs[1]
ax_timeseries.set_xlim(time[0], time[-1])
ax_timeseries.set_ylim(min(x.min(), y.min()) * 1.1, max(x.max(), y.max()) * 1.1)
ax_timeseries.set_xlabel("time (ms)")
ax_timeseries.set_ylabel("position (m)")
(line_x,) = ax_timeseries.plot([], [], label="x")
(line_y,) = ax_timeseries.plot([], [], label="y")
...
def animate(i):
    ...
    line_x.set_data(time[:i+1], x[:i+1])
    line_y.set_data(time[:i+1], y[:i+1])
    return line, line_x, line_y
