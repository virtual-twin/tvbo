...
A table that compares several dimensions rather than indexing pages — the [integration matrix](../Interoperability/index.qmd) and the [bifurcation plan](../2-specify/Models/Bifurcation/index.qmd) — stays a table, because a card carries one sentence and would drop the other columns.

## The running-example thread

`_static/running_example.yml` maps each stage of [the running example](../2-specify/RunningExample.qmd) onto the pages whose block that stage adds. `filters/running-example.lua` reads it from both ends: the pointer strip it inserts at the top of each listed page, and the stage that pointer links to. Because both come from one file, a pointer cannot outlive its stage. A page opts out with `running-example: false`.
...
Three claims that used to rot are now read from the software at render time: the [CLI command tree](../CLI/index.qmd) from the Typer app, the [model count](../2-specify/Models/index.qmd) from `Dynamics.list_db()`, and the [backend capability table](../3-generate/CodegenPhilosophy.qmd) from `tvbo.cli._backends.BACKENDS`. Prefer that pattern over a hand-kept list whenever the software already knows the answer.
