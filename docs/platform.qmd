...
The same catalogue is reachable from Python with `Dynamics.list_db()` and `Dynamics.from_db(...)`; see [Models & dynamics](Models/index.qmd).
...
### Dynamics
Pick a curated neural mass model and edit its parameters. This is the `dynamics:` block of [the running example](Specification/RunningExample.qmd).

[Open `/tvbo/configurator`](https://tvbo.charite.de/tvbo/configurator) · [docs](Models/index.qmd)
:::
...
[Open `/tvbo/configurator`](https://tvbo.charite.de/tvbo/configurator) · [docs](Specification/Network.qmd)
:::
...
[Open `/tvbo/configurator`](https://tvbo.charite.de/tvbo/configurator) · [docs](Simulation/ObservationModels.qmd)
:::
...
**Download** saves the experiment as a YAML bundle plus a companion data file for the connectome matrices; **Copy Python** puts a ready-to-run snippet on the clipboard. For full control over a run, export and use Python: [Run on a backend](Simulation/Backends.qmd) covers what the browser's backend picker is choosing between.
