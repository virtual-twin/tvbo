"""Map the tvbo package tree to the module lists quartodoc's sections file consumes."""
...
"""Section heading for each full package name — used instead of the module docstring's first line, and as the sidebar label by ``update_toc_api.py``. A sub-package gets its SHORT name, since the nesting already supplies the context."""
...
def get_docstring_title_and_description(package_init_path: str) -> tuple[str, str]:
    ...
    The title is the first non-empty line of the module's docstring. The description is the first paragraph that follows it, collapsed onto a single line. An optional RST-style underline (``====`` / ``----``) directly under the title is skipped, but is no longer *required* — plain Google/Markdown docstrings (which almost none of the TVBO packages underline) now yield a description too, instead of an empty string.
    """
...
def collect_packages_and_modules(package_name: str, package_path: str) -> dict[str, list[str]]:
    """Every importable module under each directory that is a Python package.

    Returns a mapping of ``full.package.name`` to its module names — ``.py`` files with importable names, in directories carrying an ``__init__.py``.
    """
...
def build_sections(package_name: str, package_path: str):
    ...
    Using ``kind: page`` + ``path: <subpath>/<module>`` means quartodoc writes output files to ``api/<subpath>/<module>.qmd``.  Because the path is fully qualified, modules with the same basename in different packages never collide (e.g. ``api/adapters/julia.qmd`` vs ``api/run/julia.qmd``).

    Each non-root sub-package also gets an ``index`` page that documents the package's ``__init__.py``.
    """
    sections = []
    ...
    for full_pkg in sorted(pkg_to_modules.keys()):
        ...
        # If we overrode the title and there's no separate description, use the raw docstring title as the description.
        if raw_title and title != raw_title and not description:
            description = raw_title

        # Page-style contents, so quartodoc respects the full path (relative to its `dir`, set to "." → api/).
        contents = []

        # `include_empty: true` is critical — quartodoc's auto-discovery silently skips members without docstrings. Many TVBO core classes (e.g. `SimulationExperiment`, `Dynamics`) inherit their docstrings from the generated LinkML datamodel, so they appear docstring-less to griffe's static parser. Without this flag, the class is omitted from its module's page entirely.
        auto_opts = {"include_empty": True}

        # A package index page for __init__.py; the root package is skipped, quartodoc generates api/index.qmd for it.
        if subpath:
...
def write_full_qdoc_config(sections, out_path):
    # Write a full config for the Quartodoc CLI. Point its sidebar to a different file so we can generate our own nested sidebar without it being overwritten.
    config = {
        "quartodoc": {
            "sidebar": "./_sidebar.auto.yml",  # quartodoc will write this
            "parser": "google",
            "style": "pkgdown",
            # Custom markdown renderer (../_renderer.py) that teaches quartodoc to render ``Yields:`` sections; the stock renderer errors on them.
...
def main(argv=None):
    ...
    # The generated config embeds the *docstring of every module* in the package, so the freshness input must be every ``.py`` file — not just ``tvbo/__init__.py``. Watching only the package root meant a docstring-only edit anywhere else (say a doc link in ``tvbo/run/graph.py``) was reported as "unchanged" and silently never reached the rendered API docs. The generator script itself is included so edits to ``build_sections`` are picked up too.
