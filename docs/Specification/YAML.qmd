...
Every TVB-O specification — a `Dynamics`, a `Network`, a `SimulationExperiment`, a `SimulationStudy` — is one YAML document read by one loader, so the idioms below work the same everywhere. For *what* fields each component has, see the [specification reference](index.qmd).
