...
A recipient needs `tvbo` installed and whatever data the spec references. Where the network or a fit target is external, [parameter external storage](../2-specify/Networks/Network.qmd) governs how it is pointed at rather than inlined.

On the platform, the same object is what **Share** and **Publish** move: see [the platform gallery](../1-explore/platform.qmd#-share-two-different-things) for the difference between the two, and the [REST API](https://tvbo.charite.de/docs/python-package-api) for pushing one from Python so a collaborator reproduces a run by id rather than by emailed YAML.
...
[Reproducible workflows](../3-generate/workflows/index.qmd) covers the kit's anatomy and what changes between Slurm, Snakemake and Nextflow.
...
This is the route when the value is in the *results*. [Linked experiments in a SimulationStudy](../2-specify/SimulationStudies.qmd) shows the tree a two-experiment study writes, and [Results in BIDS](../Interoperability/BIDS/BIDS_comp_modelling.md) documents where TVB-O's container layout diverges from BEP034 and why.
