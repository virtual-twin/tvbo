...
Mixing neural-mass models never leaves the mean-field regime, and it does not nest one scale inside another. The test: if you can draw the model as one graph of peer nodes, it is *heterogeneous* and belongs here. If a node must **contain** a finer network, such as a spiking patch or reservoir with its own node-space and clock, that is *multi-scale*; see [Multi-scale modelling](../BeyondMeanField/MultiScaleModelling.qmd).
:::
