...
Parameters declared in the block are substituted into both expressions, so `G` and `n` can be swept in an [exploration](../Simulation/ParameterExploration.qmd) or fitted in an [optimization](../Fitting/index.qmd) exactly like model parameters.
...
- [Bifurcation & stability](../Bifurcation/index.qmd): coupling strength is the classic bifurcation parameter for a network model.
