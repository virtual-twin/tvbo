...
A relay node is just another node on the same graph: the synapse *is* a node, peer to the neurons it connects. This is heterogeneity on one scale, not [multi-scale nesting](../BeyondMeanField/MultiScaleModelling.qmd): nothing here contains a finer network with its own node-space or clock.
:::
