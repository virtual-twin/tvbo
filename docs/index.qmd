...
`tvbo` describes a whole brain-network simulation as one typed, ontology-grounded specification. From that single source it compiles runnable code across many backends and a methods report, and grounds every entity in a knowledge graph — so studies re-run without drift and models can be compared even when they share no notation.
...
![One specification, two payoffs: from one typed specification, TVB-O compiles code and a methods report across backends (reproducible) and grounds every entity in a knowledge graph (comparable).](_static/tvbo_overview.png){fig-align="center" width="100%"}
