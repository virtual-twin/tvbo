...
A model parameter declared with `distribution: { axis: time }` is not a constant. tvboptim pre-generates a per-step random trajectory and indexes it inside the `dfun`. Other backends use a frozen scalar (`p.value`). See [Stochastic time-varying parameter inputs](Noise.qmd#stochastic-time-varying-parameter-inputs) for the YAML schema and code-generation details.
...
- [Initial Conditions & Initial State](InitialConditions.qmd): IC resolution, per-trial sampling and transient warm-up.
- [Code Generation](../ScaleOut/CodeGeneration.qmd): inspect what each backend actually emits with `print(exp.render_code('tvboptim'))`.
