...
- **User skills**: shipped in the `tvbo` wheel, installed onto your machine by `tvbo skills install`.
...
The split is by **audience**, not capability — user skills are still rich behavioural rules, just scoped to library *usage* rather than library
*development*.
...
These are the five skills `pip install tvbo && tvbo skills install` puts on your machine.
...
Turns a paper into **one declarative, tvbo-native recipe** (all or selected experiments) plus simple plotting and an honest, fully-computed report.
Composes `writing-models` and `running-simulations`, then adds the replication-specific rules:
...
- Ships an `assets/` dir — report/plot templates and a FAIR skeleton — installed alongside the skill
...
`git branch`), branch switching when explicitly requested, fetch/pull when explicitly requested.
...
- `tvbo/adapters/` wraps external simulators (TVB, Julia, NeuroML,
  PyRates, BifurcationKit, ModelingToolkit, NetworkDynamics, openMINDS, tvboptim). Contract: `tvbo/adapters/base.py`.
...
`backend_tvb`, `backend_pyrates`, `backend_tvboptim`, `backend_julia` and its sub-markers, `backend_neuroml`), plus `slow`, `julia`, `docs`, the `--run-slow` flag, and the `-n 1 --dist=loadscope` default options.
...
`.cursor/rules/`, `AGENTS.md` index, raw prompt) are **generated**. Never edit them by hand — `tvbo skills sync` (or the next install) will revert your changes.
...
A skill directory may carry an `assets/` sibling of its `SKILL.md` — helper scripts, templates, or a skeleton tree the skill tells the assistant to copy.
`replicating-studies` is the first to use it, shipping report/plot templates and a FAIR study layout.

Only the directory-shaped **claude-code** target can hold assets: `install` and `sync` mirror `assets/` next to the rendered `SKILL.md` (so an installed skill lives at `~/.claude/skills/tvbo-<name>/SKILL.md` **and**
`~/.claude/skills/tvbo-<name>/assets/`), and `uninstall` removes it. The flat targets (Cursor, Copilot, raw prompt) carry the skill body only. Byte-compiled files (`__pycache__`, `*.pyc`) are never mirrored.
