...
If you're a *user* of TVBO, you want [Installing & tuning](installing-skills.qmd) instead.
...
- **Capture non-obvious rules.** Things an LLM would predictably get wrong without this skill. Generic style advice is in `writing-code` already.
- **Cite file paths and line ranges.** They survive better than vague references like "the codegen module".
...
- Mirrors each skill's `assets/` dir, if it has one, next to that `.claude/skills/<name>/SKILL.md`.
...
If you see drift output, something downstream of canonical was hand-edited. Re-sync to fix.
...
A canonical skill may ship an `assets/` directory beside its `SKILL.md` — templates, helper scripts, or a skeleton tree the skill instructs the assistant to copy:
...
**only for the directory-shaped `claude-code` target** (the flat Cursor / Copilot / prompt outputs carry the body alone). `uninstall` removes it and `sync --check` diffs the tree. Assets ship in the wheel through the same `tvbo/skills/canonical/**` inclusion as `SKILL.md` — no extra packaging config.

Syntax-check an asset `.py` with `ast.parse` (writes nothing), **not**
`py_compile` — the latter drops a `__pycache__/*.pyc` under the canonical source. Sync ignores bytecode when mirroring, but keep the source clean.
...
You should see all five user `SKILL.md` files inside the wheel, plus any `assets/` a skill ships (e.g. `replicating-studies/assets/...`).
...
`skills/git.md`, `.claude/skills/git/SKILL.md`, and `.github/instructions/git.instructions.md`. Same content, three formats, three sources of drift. The canonical-source + renderer design replaces that with **one source, N renderings**:
...
See also: [Skill reference](skills-reference.qmd) · [`AGENTS.md`](agents-md.qmd)
