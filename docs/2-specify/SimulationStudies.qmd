...
jupyter: python3
aliases:
  - /Simulation/SimulationStudies.html
---
...
The two runs differ in one parameter, so everything else is written once and referenced. `&dyn` names a value and `*dyn` reuses it, plain YAML covered in [Writing TVB-O YAML](YAML.qmd).
...
| `figures` | [Declarative figure specs](Figures/index.qmd) that read the containers above. |
...
[Analyses](Analyses.qmd) covers this in full, and [calling your own code](CodeSource.qmd) covers where `module: mini_analysis` is found.

## Where to go next

- [Fan-out over subjects and phenotypes](FanOut.qmd): the same study run across a cohort.
- [The study directory](../Interoperability/BIDS/index.qmd): where all of this lands on disk.
- [Specify a figure](Figures/index.qmd): the grammar `figures:` is written in.
