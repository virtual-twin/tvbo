...
aliases:
  - /Bifurcation/07-NumericalContinuation.html
...
*what those backends actually do* and which numerical machinery TVBO routes your YAML system through.
...
To get past folds (where $J$ becomes singular and a naive parameter sweep fails), continuation methods reformulate the problem so that the *arclength*
$s$ along the solution branch, not the parameter $a$, is the independent variable. The augmented system is:
...
*projection* of the new point onto the previous tangent vector to a fixed step $\Delta s$. The augmented Jacobian remains non-singular *through* the fold, allowing the solver to turn around smoothly.
...
*from the same YAML*. The continuation methodology is identical across backends; only the numerics, the bifurcation-detection thresholds, and the ergonomics differ.
