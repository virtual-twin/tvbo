...
aliases:
  - /BeyondMeanField/MultiScaleModelling.html
...
*Heterogeneity* is about **which model** runs at each node; *scale* is about **how much detail** a node resolves. They are independent axes:
...
The coarse regions drive the detailed patch with their ongoing activity; the patch sends a summary of its own activity back up. You reach for this when neither level alone answers the question, when you need the whole-brain context and the local mechanism in one simulation.
...
Individual detailed models do not need the multi-scale machinery to exist. A spiking neuron, ion channel, or synapse is available on its own through the [NeuroML integration](../../../Interoperability/NeuroML/index.qmd): set `iri: neuroml:<TypeName>` on a dynamics and TVBO pulls the equations from the NeuroML2 library, so you supply only parameter values. The open work is joining such a patch to a running mean-field brain, not describing the patch.
...
- [Multi-layer networks](../../Networks/Network.qmd#advanced-multi-layer-networks)
- [Multi-scale co-simulation](../../Networks/Network.qmd#multi-scale-co-simulation), including the proxy pattern, interface variables, and how the same file maps to tvb-multiscale, JAX, and NeuroML backends
- schema slots
...
- [Defining networks](../../Networks/Network.qmd): nodes, edges, and connectomes
- [Hierarchical & surface networks](../../Networks/HierarchicalSurfaceNetworks.qmd): building a layered network, with a surface patch linked to a parent region
- [Heterogeneous nodes](../../Networks/HeterogeneousNodes.qmd) / [heterogeneous edges](../../Networks/HeterogeneousEdges.qmd): a *separate* axis (a different model per node, all on one scale) that composes with multi-scale — a nested patch can itself be heterogeneous
- [Coupling functions](../../Networks/CouplingFunctions.qmd): the rule that turns one node's state into a signal others receive

## Related goals

- [Build a connectome from tractography](../../Networks/BuildConnectomeFromTractography.qmd) for the macro-scale structural network
- [Coming from a spiking simulator?](../../../Interoperability/index.qmd) for how TVBO concepts line up with tvb-multiscale, NEST, and NeuroML
