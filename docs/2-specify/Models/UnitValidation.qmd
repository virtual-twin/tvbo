...
aliases:
  - /Models/UnitValidation.html
  - /Usage/UnitValidation.html
---

TVBO uses [SymPy's units system](https://docs.sympy.org/latest/modules/physics/units/index.html)
to handle physical units automatically. This enables correct unit conversions across different components of a simulation experiment.
...
Accepted values are `s`, `ms`, and `us` — the [`UnitEnum`](../../datamodel/enums/UnitEnum.qmd) time units. `time_scale` is accepted as a synonym for `time_unit` on input, and `dt` as a synonym for `step_size`; a synonym is read back under the canonical name. See [Integrators](../Integrators/index.qmd) for methods, noise, and coupling-stage options.
