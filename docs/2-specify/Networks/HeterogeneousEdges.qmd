---
title: "Heterogeneous Edges"
description: "Synapse-like relay nodes on the connection path: short-term plasticity modelled as its own dynamics, still a single-scale network."
aliases:
  - /Networks/HeterogeneousEdges.html
---

An edge can carry its own dynamics. Instead of a static weight, the connection between two nodes passes through a **relay node** with its own equations, here a short-term–plasticity synapse (depression, facilitation, Tsodyks–Markram). A pre-synaptic node drives three parallel synapse nodes, each filtering the signal differently before it reaches the post-synaptic node.

::: {.callout-note}
## Still single-scale, still heterogeneous

A relay node is just another node on the same graph: the synapse *is* a node, peer to the neurons it connects. This is heterogeneity on one scale, not [multi-scale nesting](../Models/BeyondMeanField/MultiScaleModelling.qmd): nothing here contains a finer network with its own node-space or clock.
:::

::: {.callout-note}
## Backend: PyRates

A relay's transmitted signal is **input-dependent**: `r_eff = r_in · x · u` reads what the synapse *receives*, not just its own state. PyRates routes such algebraic edge outputs natively, so this example runs on PyRates.

The tvboptim (JAX) engine behind [heterogeneous *nodes*](HeterogeneousNodes.qmd) transports signals derived from a node's own state; input-dependent relay outputs are a deferred milestone there. To model short-term plasticity on tvboptim today, put the synaptic state (`x`, `u`) in the *receiving* node and drive it through ordinary coupling.
:::
...
# r_eff is an input-dependent output (r_in * x * u); the PyRates backend routes it
tsodyks = Dynamics.from_string("""
name: TsodyksMarkram
description: "Short-term synaptic plasticity with depression and facilitation (canonical Tsodyks-Markram)"
parameters:
  tau_D:
    value: 200.0
    description: "Depression recovery time constant (ms)"
  tau_F:
    value: 600.0
    description: "Facilitation time constant (ms)"
  U0:
    value: 0.2
...
derived_variables:
  r_eff:
    equation:
      rhs: "r_in*x*u"
    description: "Effective synaptic transmission (release probability x resources x rate)"
output:
    - r_eff
""")
...
name: Depression
description: "Short-term synaptic depression only (fixed release probability)"
parameters:
  tau_D:
    value: 300.0
    description: "Depression recovery time constant (ms)"
  U:
    value: 0.5
    description: "Release probability (fixed)"
state_variables:
  x:
    equation:
      rhs: "(1 - x)/tau_D - U*x*r_in"
...
name: Facilitation
description: "Short-term synaptic facilitation only (full resources)"
parameters:
  tau_F:
    value: 500.0
    description: "Facilitation time constant (ms)"
