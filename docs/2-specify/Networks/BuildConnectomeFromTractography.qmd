...
aliases:
  - /Networks/BuildConnectomeFromTractography.html
  - /How-To/BuildConnectomeFromTractography.html
---

## The goal

You have diffusion-MRI tractography and want a structural connectome you can simulate on: an $N \times N$ weight matrix (streamline counts) and a matching length matrix (mean tract lengths), packaged as a TVBO `Network`. This recipe walks the full pipeline: build a parcellation atlas, run streamline-based tractography with [MRtrix3](https://www.mrtrix.org/), and save the result.

If you already have connectivity matrices and only need to wrap them, skip to [Defining Networks](Network.qmd). For a network whose regions contain finer detail, see [Model Across Scales](../Models/BeyondMeanField/MultiScaleModelling.qmd).

## Prerequisites

- **MRtrix3** installed (`tck2connectome` in PATH)
- A **tractogram** in `.tck` format, such as the dTOR group-average tractogram
...
3. **Create a TVBO `Network`**: wrap the matrices and metadata into a schema-valid HDF5 and YAML pair.
...
The `save()` method creates both files, the YAML sidecar and the HDF5 companion:
...
`scripts/create_lobar_network.py`, which creates both the SC and surface networks with caching of intermediate files:
...
- **Custom parcellations:** Any integer-labeled NIfTI in the same template space as the tractogram works, so you are not limited to TemplateFlow atlases.
- **Schema validation:** `network.save()` validates the YAML against the TVBO LinkML schema automatically.

## Next steps

- [Hierarchical & surface networks](HierarchicalSurfaceNetworks.qmd): add a surface mesh layer on top of this regional connectome, reusing the atlas built in Step 1.
- [Model Across Scales](../Models/BeyondMeanField/MultiScaleModelling.qmd): give selected regions a detailed spiking or reservoir patch.
