...
aliases:
  - /4-run/Visualization.html
  - /Simulation/Visualization.html
...
A coupling likewise plots its own transfer function. See [coupling functions](../Networks/CouplingFunctions.qmd) for what that curve means:
...
A phase plane needs values for anything the model's equations expect but the plane does not supply, namely the coupling inputs. By default these are evaluated at $0$, giving the
**isolated-node** field; pass `inputs={"c_glob": ...}` to draw the flow at a fixed coupling drive, as on the right above.

`plot_graph` has two rendering backends. The default `networkx` one is shown above;
`format="bsplot"` draws the same graph through bsplot's edge renderer, which gives curved edges and the house styling:
...
- [The experiment result](../../4-run/Results.qmd): the object being plotted, and how to index it.
- [Bifurcation & stability](../Models/Bifurcation/index.qmd): phase portraits and continuation diagrams.
