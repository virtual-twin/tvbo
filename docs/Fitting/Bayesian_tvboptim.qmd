...
Each scenario then names that observation as what its likelihood scores, and differs from the others in nothing but the width of its priors:
...
inferences:
  scenario_B:
    ...
    likelihood: {source: [observed_ts], name: Normal, sigma: 0.2}
    sampler: nuts
```
...
Under wide priors **(b)** the posterior recovers the truth: amplitude $0.41 \pm 0.06$ against a true $0.40$, excitability $0.083 \pm 0.066$ against a true $0.10$. But the two are correlated at $-0.99$, so the cloud is a line, and its width along that line is the answer, not the marginal spread of either parameter on its own.

That is what makes **(c)** and **(d)** the point of the page. Tightening the amplitude prior **(c)** does not pull the posterior towards the truth; it slides it down the ridge, to amplitude $0.37$ and excitability $0.13$. Tightening the excitability prior instead **(d)** slides it the other way, to $0.45$ and $0.037$. Each is a confident, well-mixed posterior, with all three scenarios returning $\hat{R} \approx 1.00$, and each is confident about a different place on the same ridge. The prior is doing the identifying that the recording cannot.
