...
This directory is a BIDS study dataset (`DatasetType: "study"`): the entry recipe is `Jansen1995.yaml`, its plotting callables live in `code/`, and the figures render into `docs/figures/`. See [BIDS Integration](../../Interoperability/BIDS/index.qmd) for the layout every study shares.
...
# This directory IS the study dataset, so every path below is asked of the layout record.
STUDY = "Jansen1995"
sys.path.insert(0, str(study_path("code")))
import Jansen1995_plot as jr_plot

odir = study_path("figures")
odir.mkdir(parents=True, exist_ok=True)
```

```{python}
s = SimulationStudy.from_file(f"{STUDY}.yaml")
```
