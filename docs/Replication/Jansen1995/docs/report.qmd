...
jupyter: python3
# Publication gate (see docs/scripts/update_toc_replication.py): replication
# results are embargoed until the source work is published. This study
# replicates the already-published Jansen & Rit (1995), so it may ship.
publish: true
---

Replication of the simulation experiments from @Jansen1995.

This study is a BIDS study dataset (`DatasetType: "study"`): the entry recipe is `Jansen1995.yaml` at its root, the plotting callables live in `code/`, and the figures render into `docs/figures/` beside this report. See [BIDS Integration](../../../Interoperability/BIDS/index.qmd) for the layout every study shares.
...
from tvbo.utils.study_layout import file_relpath, study_path, study_root
import bsplot

bsplot.style.use("tvbo")

# The report renders from inside the study, so every path below is asked of the layout record.
STUDY = "Jansen1995"
ROOT = study_root(Path.cwd())
sys.path.insert(0, str(study_path("code", root=ROOT)))
import Jansen1995_plot as jr_plot

odir = study_path("figures", root=ROOT)
odir.mkdir(parents=True, exist_ok=True)
```

```{python}
s = SimulationStudy.from_file(ROOT / file_relpath("recipe", study=STUDY))
```
