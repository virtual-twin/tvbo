...
*prepared simulator object* (a JAX kernel, a TVB `Simulator`, a tvboptim namespace) to drive manually, and `exp.render_code(format=…)` to inspect the generated code without running it. See [Code generation & export](../ScaleOut/CodeGeneration.qmd).
...
These do not integrate forward. They trace how fixed points and limit cycles move as a parameter varies. See [Bifurcation & stability](../Bifurcation/index.qmd).
...
See [`tvbo workflow plan`](../CLI/workflow.qmd) to preview placement, and [Reproducible workflows](../ScaleOut/index.qmd) for running the resulting DAG.
