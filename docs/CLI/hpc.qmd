...
`tvb`, which vectorizes nothing) rejects `--slurm-chunk` and points you to the Snakemake or Nextflow emitter, where each cell is its own task. See [Choosing an engine](workflow.qmd#choosing-an-engine) for that split.
