...
That writes five skills under `~/.claude/skills/tvbo-*/` (each a `SKILL.md`,
plus an `assets/` dir where the skill ships templates):
...
| `tvbo-replicating-studies` | How to replicate a published study as one recipe + report |
...
|---|---|---|
| `claude-code` | `<dest>/tvbo-<name>/SKILL.md` (+ `assets/` if the skill ships any) | `~/.claude/skills/` (user) or `./.claude/skills/` (project) |
...
By default only the five **user-audience** skills are installed (the ones in
