...
- Runs the kit's **own frozen spec**: `tvbo run spec/<exp>.yaml --backend=… --slurm-chunk=$SLURM_ARRAY_TASK_ID/N -o …`, so each array task simulates its `1/N` share of the sweep and the job is **self-contained** (no external recipe or callable modules at run time). Requires `tvbo` plus the backend runtime; provision both from the emitted `environment.yml`. (If the source recipe is preferred over the frozen spec, the fallback form is `tvbo run <recipe> --experiment "<label>" --slurm-chunk=…`.)
...
Emitted from the experiment's schema-native `environment.requirements` (skipped
if none are declared). `requirements.txt` is one pip spec per line (`pkg>=x`, or a
`source_url` verbatim); `environment.yml` is a conda env with those under `pip:`.
Provision the run env with `conda env create -f environment.yml` or
`pip install -r requirements.txt`. Both are rendered from Mako templates
(`templates/workflow/{requirements.txt,environment.yml}.mako`).
