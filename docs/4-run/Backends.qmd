...
`run(format=…)` configures delays, generates the backend's code, executes it, and returns an [`ExperimentResult`](Results.qmd). Use `exp.execute(format=…)` instead when you want the
*prepared simulator object* (a JAX kernel, a TVB `Simulator`, a tvboptim namespace) to drive manually, and `exp.render_code(format=…)` to inspect the generated code without running it. See [Code generation & export](../3-generate/CodeGeneration.qmd).
...
`--backend/-b` takes the same identifiers as Python's `format=`. Omit it to use the experiment's declared backend. See [`tvbo run`](../CLI/run.qmd).
...
Also reachable via `exp.run(format=…)` as forward/export targets (documented under [Interoperability](../Interoperability/index.qmd), not part of the planner capability matrix):
...
`render_code("cuda")` emits a RateML-style CUDA kernel, one `__global__` function per model integrating every node in a time loop with the delay history held in a ring buffer, and `exp.run("cuda")` compiles it with [PyCUDA](https://documen.tician.de/pycuda/) and
launches it across `n_work_items` parameter sets in parallel. Its reason to exist is that last part: a parameter sweep costs one launch, not one simulation per point.
...
**What it needs.** PyCUDA, an NVIDIA GPU, and a CUDA toolkit whose `nvcc` still supports that GPU's architecture. That last clause is not pedantry: PyCUDA compiles the kernel at run time and passes the detected `-arch`, so a toolkit newer than the card fails outright.
CUDA 13 dropped Volta, so a V100 (`sm_70`) under CUDA 13.3 aborts with `nvcc fatal : Unsupported gpu architecture 'sm_70'`.
...
These do not integrate forward. They trace how fixed points and limit cycles move as a parameter varies. See [Bifurcation & stability](../2-specify/Models/Bifurcation/index.qmd).
...
The CLI's workflow planner is **backend-aware**: the same `study.yaml` produces a different execution DAG per backend, because each backend can vectorize a different set of sweep axes internally. Every [`ExplorationAxis`](../2-specify/ParameterExploration.qmd) has a *kind* — `parameters`, `initial_conditions`, `noise_seed`, or `subjects`:
...
See [`tvbo workflow plan`](../CLI/workflow.qmd) to preview placement, and [Reproducible workflows](../3-generate/workflows/index.qmd) for running the resulting DAG.
...
- [The SimulationExperiment object](../2-specify/SimulationExperiments.qmd): what you're running
- [Results & output](Results.qmd): what `run()` returns
- [Code generation & export](../3-generate/CodeGeneration.qmd): inspect or save generated code
