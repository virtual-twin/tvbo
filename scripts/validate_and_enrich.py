...
def validate_hdf5():
    ...
    for f in sorted(glob.glob(os.path.join(NETWORKS_DIR, '*.h5'))):
        ...
        l = shapes.get('lengths')

        if w is None or l is None:
            problems.append((name, f'MISSING: weights={w}, lengths={l}'))
            print(f'  MISSING  {name}: found={shapes}')
        elif w != l:
            problems.append((name, f'MISMATCH: weights={w} vs lengths={l}'))
            print(f'  MISMATCH {name}: weights={w} vs lengths={l}')
