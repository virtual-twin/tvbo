...
def _reader(store):
    """A `visititems` callback that copies every dataset it visits into *store*."""
...
for a, b in pairs:
    ...
    with h5py.File(fa) as ha, h5py.File(fb) as hb:
        datasets_a = {}
        datasets_b = {}
        ha.visititems(_reader(datasets_a))
        hb.visititems(_reader(datasets_b))
        print(a)
        print(b)
        ...
        for k in sorted(set(datasets_a.keys()) & set(datasets_b.keys())):
            ...
            print(f"  {k}: same={same}, shapes={datasets_a[k].shape} vs {datasets_b[k].shape}")
        print()
