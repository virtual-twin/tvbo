...
def main(argv: list[str]) -> int:
    ...
    if base is None:
        ...
        if found.returncode != 0 or not merge_base:
            reason = found.stderr.strip() or f"{base!r} shares no history with HEAD"
            print(f"cannot compare against {base!r}: {reason} — the check did not run.")
            return 2
