...
LinkML's `gen-owl` emits a usable OWL file but stops short of the
publication-readiness items in §2.3 of the ontology restructuring plan:
...
def _augment(g: Graph, schema: dict) -> None:
    ...
    # `DimensionType#time` vs `EventType#time` vs `SamplingAxis#time` (each
    # carried `rdfs:label "time"`). The enum local-name comes from the IRI
    # fragment before `#`. Demote the bare value to `skos:notation` and
    # synthesize `"<value> (<EnumName>)"` as the human label.
    # Same treatment for slot/property IRIs nested under sub-namespaces
    ...
    # Top-level TVBO ObjectProperty / DatatypeProperty / AnnotationProperty
    # labels (`tvbo:x`, `tvbo:license`, ...) collide with InterLex / SANDS
    ...
    # ROBOT report flags `missing_label` on imported externals that LinkML
    # references but does not relabel. Inject minimal `rdfs:label`s based
    # on the IRI local name so downstream tooling has something to display.
    ...
    # `tvbo:SoftwarePackage` is declared by us but lacks a label (LinkML
    # emitted only `skos:definition`). Add the human label.
    sp = TVBO.SoftwarePackage
    if not list(g.objects(sp, RDFS.label)):
        g.add((sp, RDFS.label, Literal("Software package")))

    # Dedupe `rdfs:label` per subject. ROBOT flags `multiple_labels` even
    # when two labels carry the same lexical form but differ only in
    # language tag (e.g. `"CouplingInput"` vs `"CouplingInput"@en`). For
    # external IRIs we are not the source of truth: keep at most one
    # label, deduplicating deterministically. For TVBO IRIs, prefer the
    # language-tagged variant when it has identical lexical form.
